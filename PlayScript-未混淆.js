...
const PSLog = (...a) => console.log("[PlayScript]", ...a);
const PSErr = (...a) => console.error("[PlayScript]", ...a);
...
const PS_CLOUD_TOTAL_LIMIT = 180000;   
const PS_CLOUD_DEFAULT_CAP_KB = 50;    
const PS_BIO_MARK = "[PlayScript:1]";  
const PS_BIO_MAX = 10000;              
const PS_BIO_START = "━━━━━━【BCPS·PlayScript 数据，请勿修改】━━━━━━";   
const PS_BIO_END = "━━━━━━【BCPS·PlayScript 数据结束】━━━━━━";          
...
const PS_OBF_KEY = "PlayScript#R131#BondageClub#2026";

 
function PSObfuscate(str) {
	const key = PS_OBF_KEY;
	const utf8 = [];
	for (let i = 0; i < str.length; i++) {
		const c = str.charCodeAt(i);
		if (c < 0x80) utf8.push(c);
		else if (c < 0x800) utf8.push(0xC0 | (c >> 6), 0x80 | (c & 63));
		else if (c < 0x10000) utf8.push(0xE0 | (c >> 12), 0x80 | ((c >> 6) & 63), 0x80 | (c & 63));
		else utf8.push(0xF0 | (c >> 18), 0x80 | ((c >> 12) & 63), 0x80 | ((c >> 6) & 63), 0x80 | (c & 63));
	}
	const bytes = [];
	for (let i = 0; i < utf8.length; i++) {
		bytes.push((utf8[i] ^ key.charCodeAt(i % key.length) ^ ((i * 17 + 11) & 0xFF)) & 0xFF);
	}
	let bin = "";
...
function PSDeobfuscate(str) {
	...
	for (let i = 0; i < bin.length; i++) {
		bytes.push((bin.charCodeAt(i) ^ key.charCodeAt(i % key.length) ^ ((i * 17 + 11) & 0xFF)) & 0xFF);
	}
	let out = "";
	for (let i = 0; i < bytes.length; i++) {
		...
		else if ((b & 0xF0) === 0xE0) out += String.fromCharCode(((b & 15) << 12) | ((bytes[++i] & 63) << 6) | (bytes[++i] & 63));
		else out += String.fromCharCode(((b & 7) << 18) | ((bytes[++i] & 63) << 12) | ((bytes[++i] & 63) << 6) | (bytes[++i] & 63));
	}
	return out;
}

 
function PSStorageInfo() {
	const out = { total: 0, keys: [], top: [], self: null };
	try {
		for (let i = 0; i < localStorage.length; i++) {
			const k = localStorage.key(i);
			if (!k) continue;
			let bytes = 0;
			try { const v = localStorage.getItem(k); if (v != null) bytes = v.length * 2; } catch (e) {   }
			out.total += bytes;
			out.keys.push({ key: k, bytes });
		}
		out.keys.sort((a, b) => b.bytes - a.bytes);
		out.top = out.keys.slice(0, 12);
	} catch (e) {   }
	const mine = PSStore.baseKey();
	out.self = out.keys.find((x) => x.key === mine) || { key: mine, bytes: 0 };
	return out;
}

 
function PSCleanLSCGBackups() {
	let n = 0;
	try {
		for (let i = localStorage.length - 1; i >= 0; i--) {
			const k = localStorage.key(i);
			if (k && /^LSCG_.*_Backup$/.test(k)) {
...
const PSClamp = (v, lo, hi) => Math.min(Math.max(v, lo), hi);
const PSDeepClone = (o) => JSON.parse(JSON.stringify(o));

 
const PSEsc = (s) => String(s ?? "").replace(/[&<>\u0022\u0027]/g, (c) => ({
	"&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;",
}[c]));

let PSUidCounter = 0;
const PSUid = (p) => (p || "x") + (++PSUidCounter) + "_" + Date.now().toString(36);

 
const PSTypeMeta = {
	chat:   { icon: "", color: "#4ea1ff" },
	rp:     { icon: "", color: "#ff9f43" },
	narr:   { icon: "", color: "#a29bfe" },
...
const PSStore = {
	legacyKey: "PlayScriptData",   
	cloudKey: "PlayScriptData",    
	accountNum: null,              
	state: null,
	saveTimer: null,
	lastWarnAt: null,
	...
	baseKey() {
		return this.accountNum ? ("PlayScriptData:" + this.accountNum) : null;
	},

	emptyState() {
		return { v: 1, scripts: [], ui: { win: null, dotPos: null } };
	},
	...
	normalize(s) {
		...
		s.scripts = s.scripts.filter((x) => x && typeof x === "object" && typeof x.id === "string")
			.map((sc) => {
				sc.name = String(sc.name ?? "").slice(0, 64);
				if (!sc.name) sc.name = "未命名剧本";
				sc.keyword = String(sc.keyword ?? "").slice(0, PS_MAX_KEYWORD);
				sc.matchMode = sc.matchMode === "exact" ? "exact" : "contains";
				sc.caseSensitive = !!sc.caseSensitive;
				sc.triggerWho = ["self", "anyone", "others"].includes(sc.triggerWho) ? sc.triggerWho : "self";
				sc.cooldown = PSClamp(Number(sc.cooldown) || 0, 0, PS_MAX_DELAY);
				sc.onRetrigger = ["ignore", "restart", "queue"].includes(sc.onRetrigger) ? sc.onRetrigger : "ignore";
				sc.enabled = sc.enabled !== false;
				sc.actionEnabled = !!sc.actionEnabled;
				sc.actionName = String(sc.actionName ?? "").slice(0, 64);
				...
				sc.nodes = Array.isArray(sc.nodes)
					? sc.nodes.filter((n) => n && typeof n.id === "string").map((n) => {
						const ntype = PSTypeNames.includes(n.type) ? n.type : "chat";
						return {
							id: n.id,
							type: ntype,
							text: String(n.text ?? "").slice(0, PSNodeTextCap(ntype)),
							delay: PSClamp(Number(n.delay) || 0, 0, PS_MAX_DELAY),
							enabled: n.enabled !== false,
							force: !!n.force,
							outfitTarget: n.outfitTarget === "self" ? "self" : "target",
							outfitClothes: PSOutfitFlagsFromNode(n).outfitClothes,
							outfitBody: PSOutfitFlagsFromNode(n).outfitBody,
							outfitItems: PSOutfitFlagsFromNode(n).outfitItems,
							...
							judgeType: PSNormalizeJudgeType(n.judgeType),
							...
							countMode: n.countMode === "interact" ? "interact" : (n.countMode === "orgasm" ? "orgasm" : (n.countMode === "node" ? "node" : "script")),
		...
		s.ui = { win: (ui.win && typeof ui.win === "object") ? ui.win : null, dotPos: (ui.dotPos && typeof ui.dotPos === "object") ? ui.dotPos : null };
		const capN = Number(s.cloudCapKB);
		s.cloudCapKB = (isFinite(capN) && capN >= 0) ? capN : PS_CLOUD_DEFAULT_CAP_KB;
		return s;
	},
	...
	load() {
		...
		let raw = PSStorage.get(key);
		if (raw === null) {
			
			const legacy = PSStorage.get(this.legacyKey);
			if (legacy !== null) { raw = legacy; PSStorage.remove(this.legacyKey); }
		}
		if (raw !== null) {
	...
	decode(raw) {
		...
		} else if (typeof raw === "string" && raw.indexOf("z:") === 0) {
			try {
				if (typeof LZString !== "undefined" && LZString && typeof LZString.decompressFromBase64 === "function") {
					const d = LZString.decompressFromBase64(raw.slice(2));
					if (typeof d === "string" && d) text = d;
	...
	ensureAccount(num, cloudState) {
		if (!Number.isInteger(num) || num <= 0) return;
		if (this.accountNum !== num) {
			const prevUnbound = (this.accountNum === null && this.state && Array.isArray(this.state.scripts))
				? this.state.scripts.filter((s) => s && s.id)
				: null;
			const demo = PSMakeDemoScript();
			const userScripts = prevUnbound ? prevUnbound.filter((s) => s.id !== "demo1" || JSON.stringify(s) !== JSON.stringify(demo)) : null;
			if (this.saveTimer) { clearTimeout(this.saveTimer); this.saveTimer = null; }
			this.accountNum = num;
			this.load();
			if (cloudState) {
				
				this.state = this.normalize(cloudState);
				const key = this.baseKey();
				if (key) PSStorage.set(key, this.encode(this.state));
			}
	...
	cloudCapKB() {
		const n = Number(this.state && this.state.cloudCapKB);
		return (isFinite(n) && n >= 0) ? n : PS_CLOUD_DEFAULT_CAP_KB;
	},
	...
	saveBio(cloudAccepted) {
		try {
			...
			ServerAccountUpdate.QueueData({ Description: payload });
		} catch (e) {   }
	},
...
function PSBioPack(scripts) {
	try {
		if (!scripts || !scripts.length) return "";
		if (typeof LZString === "undefined" || !LZString || typeof LZString.compressToUTF16 !== "function") return "";
		const json = JSON.stringify({ v: 1, scripts });
		return PS_BIO_MARK + PSObfuscate(LZString.compressToUTF16(json));
	} catch (e) { return ""; }
}
...
function PSBioUnpack(desc) {
	try {
		...
		if (section == null) {
			...
			const idx = desc.indexOf(PS_BIO_MARK);
		...
		const json = LZString.decompressFromUTF16(dec);
		if (!json) return null;
...
function PSMeasureDataSize(data) {
	const record = {};
	const entries = [];
	...
	for (const key of Object.keys(src)) {
		let bytes = NaN;
		try {
			bytes = PSUTF8Bytes(JSON.stringify(src[key] ?? ""));
		} catch (e) {   }
		entries.push([key, bytes]);
	}
	entries.sort((a, b) => {
		const na = Math.round(a[1] / 100);
		const nb = Math.round(b[1] / 100);
		if (na === nb || (isNaN(na) && isNaN(nb))) return String(a[0]).localeCompare(String(b[0]));
		if (isNaN(na) || isNaN(nb)) return isNaN(nb) ? -1 : 1;
		return nb - na;
	});
...
function PSByteToKB(nByte) {
	return Math.round(nByte / 100) / 10;
}

 
function PSCloudSelfBytes() {
	try {
	...
	for (const [top, val] of Object.entries(src)) {
		if ((top === "ExtensionSettings" || top === "OnlineSharedSettings" || top === "OnlineSettings") && val && typeof val === "object") {
			for (const [k, v] of Object.entries(val)) {
				out.push({ label: PSCloudNestedKeyLabel(top, k), bytes: PSUTF8Bytes(JSON.stringify(v ?? "")), key: top + "." + k, top, sub: k, value: v });
			}
		} else {
			out.push({ label: PSCloudKeyLabel(top), bytes: PSUTF8Bytes(JSON.stringify(val ?? "")), key: top, top, sub: null, value: val });
		}
	}
	out.sort((a, b) => {
...
function PSCloudInfo() {
	...
	return {
		...
		self: (snap.self != null) ? snap.self : PSCloudSelfBytes(),
...
function PSMakeDemoScript() {
	return {
		...
		nodes: [
			...
			{ id: "demo1n2", type: "narr", text: "一阵清甜的果香在空气中弥漫开来", delay: 2, enabled: true },
			{ id: "demo1n3", type: "chat", text: "这苹果可甜了，要尝尝吗？", delay: 1, enabled: true },
			{ id: "demo1n4", type: "action", text: "<nick>咔嚓咬了一口苹果，汁水四溅", delay: 1.5, enabled: true },
		],
	};
}
...
function PSFindScript(id) {
	if (!PSStore.state) return null;
	return PSStore.state.scripts.find((s) => s.id === id) || null;
}

function PSAddScript(name, keyword) {
	if (PSStore.state.scripts.length >= PS_MAX_SCRIPTS) { PSToast(PST("toastMaxScripts", PS_MAX_SCRIPTS)); return null; }
	const s = {
		id: PSUid("s"),
		name: String(name ?? "").slice(0, 64) || "未命名剧本",
		keyword: String(keyword ?? "").slice(0, PS_MAX_KEYWORD),
		matchMode: "contains",
		caseSensitive: false,
		triggerWho: "self",
		cooldown: 10,
	...
	PSStore.state.scripts.push(s);
	PSStore.requestSave();
	PSToast(PST("toastScriptAdded"));
	return s;
}

function PSDeleteScript(id) {
	const before = PSStore.state.scripts.length;
	PSStore.state.scripts = PSStore.state.scripts.filter((s) => s.id !== id);
	if (PSStore.state.scripts.length === before) return false;
	if (PSUI.selScriptId === id) { PSUI.selScriptId = null; PSUI.selNodeId = null; }
	PSStore.requestSave();
	PSToast(PST("toastScriptDeleted"));
	return true;
}

function PSDuplicateScript(id) {
	...
	PSStore.requestSave();
	PSToast(PST("toastDup"));
	return copy;
}

 
function PSUpdateScript(id, patch) {
	const sc = PSFindScript(id);
	if (!sc || !patch || typeof patch !== "object") return false;
	if ("name" in patch) sc.name = String(patch.name ?? "").slice(0, 64) || "未命名剧本";
	if ("keyword" in patch) sc.keyword = String(patch.keyword ?? "").slice(0, PS_MAX_KEYWORD);
	if ("matchMode" in patch) sc.matchMode = patch.matchMode === "exact" ? "exact" : "contains";
	if ("caseSensitive" in patch) sc.caseSensitive = !!patch.caseSensitive;
	if ("triggerWho" in patch) sc.triggerWho = ["self", "anyone", "others"].includes(patch.triggerWho) ? patch.triggerWho : "self";
	if ("cooldown" in patch) sc.cooldown = PSClamp(Number(patch.cooldown) || 0, 0, PS_MAX_DELAY);
	if ("onRetrigger" in patch) sc.onRetrigger = ["ignore", "restart", "queue"].includes(patch.onRetrigger) ? patch.onRetrigger : "ignore";
	if ("enabled" in patch) sc.enabled = patch.enabled !== false;
	if ("actionEnabled" in patch) sc.actionEnabled = !!patch.actionEnabled;
	if ("actionName" in patch) sc.actionName = String(patch.actionName ?? "").slice(0, 64);
...
function PSAddNode(scriptId, node, atIdx) {
	const sc = PSFindScript(scriptId);
	if (!sc) return null;
	if (sc.nodes.length >= PS_MAX_NODES) { PSToast(PST("toastMaxNodes", PS_MAX_NODES)); return null; }
	const n = {
		id: PSUid("n"),
		type: PSTypeNames.includes(node && node.type) ? node.type : "chat",
		text: String((node && node.text) ?? "").slice(0, PSNodeTextCap(PSTypeNames.includes(node && node.type) ? node.type : "chat")),
		delay: PSClamp(Number((node && node.delay) || 0) || 0, 0, PS_MAX_DELAY),
		enabled: !node || node.enabled !== false,
		force: !!(node && node.force),
		outfitTarget: (node && node.outfitTarget === "self") ? "self" : "target",
		outfitClothes: PSOutfitFlagsFromNode(node).outfitClothes,
		outfitBody: PSOutfitFlagsFromNode(node).outfitBody,
		outfitItems: PSOutfitFlagsFromNode(node).outfitItems,
		outfitCos: PSOutfitFlagsFromNode(node).outfitCos,
		...
		judgeType: PSNormalizeJudgeType(node && node.judgeType),
		...
		countMode: (node && node.countMode === "interact") ? "interact" : ((node && node.countMode === "orgasm") ? "orgasm" : ((node && node.countMode === "node") ? "node" : "script")),
...
function PSDeleteNode(scriptId, nodeId) {
	const sc = PSFindScript(scriptId);
	if (!sc) return false;
	const node = sc.nodes.find((n) => n.id === nodeId);
	if (!node) return false;
	
	const sub = (node.type === "judge") ? PSJudgeBranchSubtree(sc, node) : new Set();
	const before = sc.nodes.length;
	sc.nodes = sc.nodes.filter((n) => n.id !== nodeId && !sub.has(n.id));
	if (sc.nodes.length === before) return false;
	
	sc.nodes.forEach((n) => {
		if (n.nextId && (n.nextId === nodeId || sub.has(n.nextId))) n.nextId = null;
		if (n.yesId && (n.yesId === nodeId || sub.has(n.yesId))) n.yesId = null;
		if (n.noId && (n.noId === nodeId || sub.has(n.noId))) n.noId = null;
...
function PSMoveNodeTo(sc, nodeId, toIdx) {
	...
	const node = sc.nodes[from];
	sc.nodes.splice(from, 1);
	sc.nodes.splice(PSClamp(toIdx, 0, sc.nodes.length), 0, node);
	PSStore.requestSave();
	return true;
}

function PSUpdateNode(scriptId, nodeId, patch) {
	const sc = PSFindScript(scriptId);
	if (!sc) return false;
	const n = sc.nodes.find((x) => x.id === nodeId);
	if (!n || !patch || typeof patch !== "object") return false;
	if ("type" in patch && PSTypeNames.includes(patch.type)) n.type = patch.type;
	if ("text" in patch) n.text = String(patch.text ?? "").slice(0, PSNodeTextCap(n.type));
	if ("delay" in patch) n.delay = PSClamp(Number(patch.delay) || 0, 0, PS_MAX_DELAY);
	if ("enabled" in patch) n.enabled = patch.enabled !== false;
	if ("force" in patch) n.force = !!patch.force;
	if ("outfitTarget" in patch) n.outfitTarget = patch.outfitTarget === "self" ? "self" : "target";
	if ("outfitMode" in patch) {
		
		const f = PSOutfitFlagsFromNode(patch);
		n.outfitClothes = f.outfitClothes;
		n.outfitBody = f.outfitBody;
		n.outfitItems = f.outfitItems;
		n.outfitCos = f.outfitCos;
	}
	if ("outfitClothes" in patch) n.outfitClothes = patch.outfitClothes !== false;
	if ("outfitBody" in patch) n.outfitBody = patch.outfitBody !== false;
	if ("outfitItems" in patch) n.outfitItems = patch.outfitItems !== false;
	if ("outfitCos" in patch) n.outfitCos = patch.outfitCos !== false;
	...
	if ("judgeType" in patch) n.judgeType = PSNormalizeJudgeType(patch.judgeType);
	...
	if ("countMode" in patch) n.countMode = patch.countMode === "interact" ? "interact" : (patch.countMode === "orgasm" ? "orgasm" : (patch.countMode === "node" ? "node" : "script"));
...
const PSEscapeRegExp = (s) => String(s).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");

 
const PS_WORD_CHAR = /[\w\u4e00-\u9fff\u3400-\u4dbf\u3040-\u30ff\uac00-\ud7af]/;

 
function PSMatchInfo(text, sc) {
	if (!text || !sc || !sc.keyword) return null;
	const a = String(text).trim();
	const kw = String(sc.keyword);
	if (!kw) return null;
	const hay = sc.caseSensitive ? a : a.toLowerCase();
	const needle = sc.caseSensitive ? kw : kw.toLowerCase();
	if (sc.matchMode === "exact") {
		return hay === needle ? { start: 0, end: hay.length } : null;
	}
	const re = new RegExp(PSEscapeRegExp(needle), "g");
	let m;
	while ((m = re.exec(hay))) {
		const start = m.index;
		...
		if (before && PS_WORD_CHAR.test(before)) continue;   
		if (after && PS_WORD_CHAR.test(after)) continue;     
		return { start, end };
	}
	return null;
}

function PSMatchText(text, sc) {
	return !!PSMatchInfo(text, sc);
}

 
function PSTriggerTargetOf(text, sc, match) {
	const a = String(text);
	const left = a.slice(0, match.start);
	const right = a.slice(match.end);
	...
	const push = (arr) => {
		...
		for (const C of arr) {
			if (C && typeof C.MemberNumber === "number" && (typeof C.Name === "string" || typeof C.Nickname === "string")) members.push(C);
		}
	};
	try { push(typeof ChatRoomCharacter !== "undefined" ? ChatRoomCharacter : null); } catch (e) {   }
	try { push(typeof PrivateCharacter !== "undefined" ? PrivateCharacter : null); } catch (e) {   }
	if (typeof Player !== "undefined" && Player && typeof Player.MemberNumber === "number" && typeof Player.Name === "string" && Player.Name) members.push(Player);
	if (!members.length) return null;
	const SEP = "[\\s,，。.!！?？、;；:：\"'（）()【】\\[\\]<>《》…~～|·•—－]";
	const sepTrim = new RegExp("^" + SEP + "+|" + SEP + "+$", "g");
	const full = joined.replace(sepTrim, "").trim();
	const chunkRe = new RegExp("[^" + SEP + "]+", "g");
	const chunks = [];
	for (const cm of joined.matchAll(chunkRe)) chunks.push(cm[0]);
	...
	const matchOne = (candidate) => {
		...
		for (const C of members) {
			if (eq(C.Name, candidate) || eq(C.Nickname, candidate)) {
				return { num: C.MemberNumber, name: C.Nickname || C.Name };
			}
		}
		if (/^\d+$/.test(candidate)) {
			const num = Number(candidate);
			for (const C of members) {
				if (typeof C.MemberNumber === "number" && C.MemberNumber === num) return { num: C.MemberNumber, name: C.Nickname || C.Name };
			}
		}
		return null;
	};
	
	let found = matchOne(full);
	if (!found) {
...
function PSTriggerTargetAttempt(text, sc, match) {
	const a = String(text);
	const left = a.slice(0, match.start);
	const right = a.slice(match.end);
	const SEP = "[\\s,，。.!！?？、;；:：\"'（）()【】\\[\\]<>《》…~～|·•—－]";
	const sepTrim = new RegExp("^" + SEP + "+|" + SEP + "+$", "g");
	const l = left.replace(sepTrim, "").trim();
	const r = right.replace(sepTrim, "").trim();
	if (!l && !r) return null;
	if (l && r) return null;   
	return l || r;
}
...
function PSImportJSON(text) {
	let data = null;
	try { data = JSON.parse(String(text ?? "")); } catch (e) { return null; }
	if (!data || typeof data !== "object") return null;
	const arr = Array.isArray(data) ? data
		: (Array.isArray(data.scripts) ? data.scripts
			: (data.data && Array.isArray(data.data.scripts) ? data.data.scripts : null));
	if (!arr) return null;
	const state = PSStore.state;
	const byId = {};
	state.scripts.forEach((s) => { byId[s.id] = s; });
	let added = 0, replaced = 0, bad = 0;
	for (const raw of arr) {
		if (!raw || typeof raw !== "object") { bad++; continue; }
		if (typeof raw.id !== "string") raw.id = PSUid("s");   
		const norm = PSStore.normalize({ v: 1, scripts: [raw] }).scripts[0];
		if (!norm) { bad++; continue; }
		if (byId[norm.id]) { Object.assign(byId[norm.id], norm); replaced++; }
		else {
			if (state.scripts.length >= PS_MAX_SCRIPTS) { bad++; continue; }
			state.scripts.push(norm); byId[norm.id] = norm; added++;
		}
	}
	PSStore.requestSave();
...
function PSClearAll() {
	PSQueue = [];
	if (PSActive) PSAbortCore(true, "clear");
	PSStore.state.scripts = [];
	PSStore.requestSave();
	PSToast(PST("toastCleared"));
}
...
function PSNormalizeText(t) {
	return String(t ?? "").replace(/\r\n?/g, "\n").trim().slice(0, PS_MAX_TEXT);
}

 
function PSNodeTextCap(type) {
	return type === "outfit" ? PS_MAX_CODE : PS_MAX_TEXT;
}

 
function PSNormalizeCode(t) {
	return String(t ?? "").replace(/\r\n?/g, "\n").trim().slice(0, PS_MAX_CODE);
}
...
function PSCharDisplayName(C) {
	...
	try {
		if (typeof CharacterNickname === "function") {
			const n = CharacterNickname(C);
			if (n && String(n).trim()) return String(n);
		}
	} catch (e) {   }
	if (C.Name && String(C.Name).trim()) return String(C.Name);
	return (C.MemberNumber !== undefined && C.MemberNumber !== null) ? ("#" + C.MemberNumber) : null;
}

 
function PSCharObjectOf(num) {
...
function PSCharIsSelf(C) {
	if (!C) return false;
	try { if (typeof C.IsPlayer === "function") return !!C.IsPlayer(); } catch (e) {   }
	if (typeof Player !== "undefined" && Player && typeof C.MemberNumber === "number" && Player.MemberNumber === C.MemberNumber) return true;
	return false;
}

 
function PSTargetInRoom(num) {
...
function PSNormalizeJudgeType(v) {
...
function PSNormalizePlayerIds(arr) {
	if (!Array.isArray(arr)) return [];
	const out = [];
	for (const v of arr) {
		const n = Number(v);
...
function PSNormalizePlayerBranches(arr) {
	if (!Array.isArray(arr)) return [];
	const out = [];
	for (const b of arr) {
		if (!b || typeof b !== "object") continue;
		out.push({
			id: typeof b.id === "string" ? b.id : PSUid("pb"),
...
function PSPortTarget(node, port) {
	if (!node || !port) return null;
	if (port === "next") return node.nextId || null;
	if (port === "yes") return node.yesId || null;
	if (port === "no") return node.noId || null;
	if (port === "else") return node.elseId || null;
	const pm = /^player:(.+)$/.exec(String(port));
	if (pm && Array.isArray(node.playerBranches)) {
		const b = node.playerBranches.find((x) => x.id === pm[1]);
...
function PSNormalizeRelationBranches(arr) {
	if (!Array.isArray(arr)) return [];
	const out = [];
	const seenRel = new Set();
	for (const b of arr) {
		if (!b || typeof b !== "object") continue;
		const rel = PS_RELATION_ORDER.includes(b.rel) ? b.rel : null;
		if (!rel || seenRel.has(rel)) continue;
		seenRel.add(rel);
		out.push({
			id: typeof b.id === "string" ? b.id : PSUid("rb"),
			rel,
			nodeId: (typeof b.nodeId === "string" && b.nodeId) ? b.nodeId : null,
		});
		if (out.length >= PS_MAX_JUDGE_BRANCHES) break;
...
function PSJudgeRoll(node, targetNum, scriptId) {
	try {
		if (node && node.judgeType === "player") {
			const me = (typeof Player !== "undefined" && Player && Number.isInteger(Player.MemberNumber)) ? Player.MemberNumber : null;
			const t = (targetNum === undefined) ? (PSActive ? PSActive.targetNum : null) : targetNum;
			const other = (Number.isInteger(t) && t > 0 && t !== me) ? t : null;
			if (other) {
				const branches = PSNormalizePlayerBranches(node.playerBranches);
				const br = branches.find((b) => b.ids && b.ids.includes(other));
				if (br) return { yes: false, label: "#" + other, targetId: br.nodeId || null };
				if (node.elseId) return { yes: false, label: "#" + other, targetId: node.elseId };
			}
			return { yes: false, label: other ? ("#" + other) : "无目标", targetId: null };
		}
		if (node && node.judgeType === "relation") {
			const me = (typeof Player !== "undefined" && Player && Number.isInteger(Player.MemberNumber)) ? Player.MemberNumber : null;
			const t = (targetNum === undefined) ? (PSActive ? PSActive.targetNum : null) : targetNum;
			const other = (Number.isInteger(t) && t > 0 && t !== me) ? t : null;
...
function PSCounterRecordNode(scriptId, nodeId) {
	const sc = PSFindScript(scriptId);
	if (!sc) return 0;
	const n = sc.nodes.find((x) => x.id === nodeId);
	if (!n || n.type !== "judge" || n.judgeType !== "count" || n.countMode !== "node") return 0;
...
function PSJudgeAddPlayerBranch(scriptId, nodeId) {
	const sc = PSFindScript(scriptId);
	if (!sc) return null;
	const node = sc.nodes.find((n) => n.id === nodeId);
	if (!node || node.type !== "judge") return null;
	...
	branches.push(branch);
	PSUpdateNode(scriptId, nodeId, { playerBranches: branches });
	return branch;
}

 
function PSJudgeAddRelationBranch(scriptId, nodeId) {
	const sc = PSFindScript(scriptId);
	if (!sc) return null;
	const node = sc.nodes.find((n) => n.id === nodeId);
	if (!node || node.type !== "judge") return null;
	...
	const n = PSAddNode(scriptId, { type: "chat", text: PST("judgeBranchPh", branches.length + 1), delay: 0, enabled: true });
	if (!n) return null;
	const branch = { id: PSUid("rb"), rel, nodeId: n.id };
	branches.push(branch);
	PSUpdateNode(scriptId, nodeId, { relationBranches: branches });
	return branch;
}
...
function PSJudgeBranchSubtree(sc, judge) {
	...
	const out = new Set();
	const stack = [];
	const push = (id) => { if (id && byId.has(id)) stack.push(id); };
	if (judge.yesId) push(judge.yesId);
	if (judge.noId) push(judge.noId);
	...
	while (stack.length) {
		...
		if (!n) continue;
		out.add(id);
		if (n.nextId) push(n.nextId);
		if (n.type === "judge") {
			if (n.yesId) push(n.yesId);
			if (n.noId) push(n.noId);
...
function PSJudgeSwitchType(scriptId, nodeId, newType) {
	const sc = PSFindScript(scriptId);
	if (!sc) return false;
	const node = sc.nodes.find((n) => n.id === nodeId);
	if (!node || node.type !== "judge") return false;
	if (node.judgeType === newType) return true;
	const sub = PSJudgeBranchSubtree(sc, node);
	if (sub.size) {
		sc.nodes = sc.nodes.filter((n) => !sub.has(n.id));
		
		sc.nodes.forEach((n) => {
			if (n.nextId && sub.has(n.nextId)) n.nextId = null;
			if (n.yesId && sub.has(n.yesId)) n.yesId = null;
			if (n.noId && sub.has(n.noId)) n.noId = null;
	...
	const mk = (text) => PSAddNode(scriptId, { type: "chat", text, delay: 0, enabled: true });
	if (newType === "dice") {
...
function PSNodeConnect(scriptId, nodeId, port, targetId) {
	...
	let patch;
	if (port === "yes") patch = { yesId: targetId };
	else if (port === "no") patch = { noId: targetId };
	else if (port === "else") patch = { elseId: targetId };
		...
		patch = { playerBranches: branches.map((b) => b.id === bid ? Object.assign({}, b, { nodeId: targetId }) : b) };
	} else if (typeof port === "string" && port.indexOf("relation:") === 0) {
...
function PSNodeDisconnect(scriptId, nodeId, port) {
	...
	if (!node) return false;
	let patch;
	if (port === "yes") patch = { yesId: null };
	else if (port === "no") patch = { noId: null };
	else if (port === "else") patch = { elseId: null };
	else if (typeof port === "string" && port.indexOf("player:") === 0) {
		const bid = port.slice("player:".length);
		const branches = PSNormalizePlayerBranches(node.playerBranches);
		patch = { playerBranches: branches.map((b) => b.id === bid ? Object.assign({}, b, { nodeId: null }) : b) };
	} else if (typeof port === "string" && port.indexOf("relation:") === 0) {
		const bid = port.slice("relation:".length);
		const branches = PSNormalizeRelationBranches(node.relationBranches);
...
function PSSendChat(text) {
	text = PSNormalizeText(text);
	if (!text || !PSCanSend()) return false;
	if (typeof ChatRoomSendChatMessage === "function") {
		try { return ChatRoomSendChatMessage(text) !== false; } catch (e) { PSErr("PSSendChat", e); }
	}
	try {
		ServerSend("ChatRoomChat", { Content: text, Type: "Chat", Dictionary: [] });
		return true;
	} catch (e) { PSErr("PSSendChat", e); return false; }
}

 
function PSEmoteBlocked() {
	if (typeof ChatRoomOwnerPresenceRule === "function") {
		try { return !!ChatRoomOwnerPresenceRule("BlockEmote", null); } catch (e) { return false; }
	}
	return false;
}

 
function PSChatReplyId() {
	try {
		if (typeof ChatRoomMessageGetReplyId === "function") return ChatRoomMessageGetReplyId();
	} catch (e) {   }
	return undefined;
}

 
function PSSendEmoteDirect(content) {
	if (!content || !PSCanSend()) return false;
	if (PSEmoteBlocked()) return false;
	try {
		if (typeof ChatRoomGenerateChatRoomChatMessage === "function" && typeof ServerSend === "function") {
			ServerSend("ChatRoomChat", ChatRoomGenerateChatRoomChatMessage("Emote", content, PSChatReplyId()));
			return true;
		}
	} catch (e) { PSErr("PSSendEmoteDirect", e); }
	
	try {
		ServerSend("ChatRoomChat", { Content: content, Type: "Emote", Dictionary: [] });
		return true;
	} catch (e) { PSErr("PSSendEmoteDirect", e); return false; }
}

 
function PSSendRp(text) {
	text = PSNormalizeText(text).replace(/^\*+/, "");
	if (!text) return false;
	return PSSendEmoteDirect(text);
}

 
function PSSendNarr(text) {
	text = PSNormalizeText(text).replace(/^\*+/, "");
	if (!text) return false;
	return PSSendEmoteDirect("*" + text);
}
...
function PSSendAction(text) {
	...
	try {
		...
		return true;
	} catch (e) { PSErr("PSSendAction", e); return false; }
}

 
let PSLeaveNodeActive = false;

 
function PSSendLeave(node) {
	const force = !!(node && node.force);
	const leaveNow = () => {
		if (typeof ChatRoomLeave === "function") ChatRoomLeave();
		if (typeof CommonSetScreen === "function") {
			try { CommonSetScreen("Online", "ChatSearch"); } catch (e) {   }
		}
		return true;
	};
	if (!force) {
		try {
			if (typeof ChatRoomAttemptLeave === "function") {
				PSLeaveNodeActive = true;
				ChatRoomAttemptLeave();
				PSLeaveNodeActive = false;
				PSLog("演出结束（台词节点离开房间）");
				return true;
			}
		} catch (e) { PSErr("PSSendLeave", e); }
	}
	...
	try {
		...
		const ok = leaveNow();
		PSLeaveNodeActive = false;
		if (ok) PSLog(force ? "演出结束（强行离开房间）" : "演出结束（台词节点离开房间）");
		return ok;
	} catch (e) {
		PSLeaveNodeActive = false;
		PSErr("PSSendLeave", e);
...
function PSSendJudge(node) {
	if (node && node.judgeType === "count" && node.countMode === "node" && PSActive) {
		node.countValue = PSCounterRecordNode(PSActive.scriptId, node.id);
	}
...
function PSOutfitClassOf(b, family) {
	let g = null;
	try {
		if (typeof AssetGroupGet === "function") g = AssetGroupGet(family, b && b.Group);
	} catch (e) {   }
	if (!g) return "none";   
	if (g.Category === "Appearance") {
		if (!g.Clothing) return "body";
		if (g.AllowNone === false) return "none";   
		return g.BodyCosplay ? "cos" : "clothes";
	}
	if (g.Category === "Item" && !g.BodyCosplay) return "items";
	return "none";
}

 
function PSOutfitFlagsFromNode(n) {
	if (n && typeof n === "object" &&
		("outfitClothes" in n || "outfitBody" in n || "outfitItems" in n || "outfitCos" in n)) {
		return {
			outfitClothes: n.outfitClothes !== false,
			outfitBody: n.outfitBody !== false,
			outfitItems: n.outfitItems !== false,
			outfitCos: n.outfitCos !== false,
		};
	}
	const mode = (n && n.outfitMode) ? n.outfitMode : "all";
	if (mode === "clothes") return { outfitClothes: true, outfitBody: false, outfitItems: false, outfitCos: false };
	if (mode === "body") return { outfitClothes: true, outfitBody: true, outfitItems: false, outfitCos: false };
	if (mode === "items") return { outfitClothes: false, outfitBody: false, outfitItems: true, outfitCos: false };
	return { outfitClothes: true, outfitBody: true, outfitItems: true, outfitCos: true };
}

 
function PSOutfitClassAllowed(cls, flags) {
	if (cls === "clothes") return flags.outfitClothes;
	if (cls === "body") return flags.outfitBody;
	if (cls === "cos") return flags.outfitCos;
	if (cls === "items") return flags.outfitItems;
	return false;   
}

 
function PSDecodeOutfitCode(code) {
	let text = String(code || "").trim();
	if (!text) return null;
	try {
		if (text[0] !== "[") {
			if (typeof LZString === "undefined" || typeof LZString.decompressFromBase64 !== "function") return null;
			const decompressed = LZString.decompressFromBase64(text);
			if (!decompressed || typeof decompressed !== "string") return null;
			text = decompressed.trim();
		}
		...
		for (const it of arr) {
			...
			if (typeof it.Group !== "string" || typeof it.Name !== "string") continue;
			const b = { Group: it.Group, Name: it.Name };
			if (it.Color !== undefined) b.Color = it.Color;
			if (it.Property !== undefined) b.Property = it.Property;
			if (it.Craft !== undefined) b.Craft = it.Craft;
			if (it.Difficulty !== undefined) b.Difficulty = it.Difficulty;
			out.push(b);
		}
		return out.length ? out : null;
	} catch (e) { PSErr("PSDecodeOutfitCode", e); return null; }
}
...
function PSApplyOutfit(C, bundle, opts) {
	...
	const clearOnly = opts.itemsMode === "override" && Array.isArray(opts.selGroups) && opts.selGroups.length > 0;
	if (!C || !Array.isArray(bundle) || (!bundle.length && !clearOnly)) return "fail";
	const family = (typeof C.AssetFamily === "string" && C.AssetFamily) ? C.AssetFamily : "Female3DCG";
	const src = (typeof Player !== "undefined" && Player && typeof Player.MemberNumber === "number") ? Player.MemberNumber : ((typeof C.MemberNumber === "number") ? C.MemberNumber : null);
	const fromSelf = src != null && typeof C.MemberNumber === "number" && C.MemberNumber === src;

	
	let params = null;
	if (typeof ValidationCreateDiffParams === "function") {
		try { params = ValidationCreateDiffParams(C, src); } catch (e) { params = null; }
	}
	
	
	const addOnlyItemGroups = new Set();
	if (opts.itemsMode !== "override") {
		for (const b of bundle) {
			if (b && typeof b.Group === "string" && PSOutfitClassOf(b, family) === "items") addOnlyItemGroups.add(b.Group);
		}
	}
	...
	let didPreremove = false;
	if (params && typeof ValidationCanRemoveItem === "function" && Array.isArray(C.Appearance)) {
		try {
	...
	PSLastOutfitBlocked = [];
	const skipBlocked = (b, why) => {
		blocked++;
		const label = (b && typeof b.Group === "string") ? (b.Group + "/" + (typeof b.Name === "string" ? b.Name : "?")) : "?";
		PSLastOutfitBlocked.push(label + (why ? "（" + why + "）" : ""));
	};
	for (const b of bundle) {
		if (!b || typeof b.Group !== "string" || typeof b.Name !== "string") { skipBlocked(b, "无效条目"); continue; }
		
		
		const existing = (Array.isArray(C.Appearance) && C.Appearance.find((a) => a && a.Asset && a.Asset.Group && a.Asset.Group.Name === b.Group)) || null;
		if (existing) {
			const bCls = PSOutfitClassOf(b, family);
			if (bCls === "items" && opts.itemsMode !== "override") { skipBlocked(b, "已有道具（添加模式不覆盖）"); continue; }
			if (!fromSelf && existing.Property && (existing.Property.Lock || existing.Property.LockedBy)) { skipBlocked(b, "旧部件锁住不可移除"); continue; }
			if (didPreremove) { skipBlocked(b, "旧部件锁住不可移除"); continue; }
		}
		if (!fromSelf) {
			try {
				
				if (typeof ValidationIsItemBlockedOrLimited === "function" && ValidationIsItemBlockedOrLimited(C, src, b.Group, b.Name)) { skipBlocked(b, "受限/封禁清单"); continue; }
				if (typeof InventoryIsPermissionBlocked === "function" && InventoryIsPermissionBlocked(C, b.Name, b.Group)) { skipBlocked(b, "权限锁定"); continue; }
				
				const G = (typeof AssetGroupGet === "function") ? AssetGroupGet(family, b.Group) : null;
				if (G && C.OnlineSharedSettings && typeof C.OnlineSharedSettings === "object") {
					if (G.BodyCosplay && C.OnlineSharedSettings.BlockBodyCosplay) { skipBlocked(b, "对方禁cosplay"); continue; }
					if (G.Category === "Appearance" && !G.Clothing && !C.OnlineSharedSettings.AllowFullWardrobeAccess) { skipBlocked(b, "对方未开完整衣柜权限"); continue; }
				}
			} catch (e) {   }
		}
		const A = (typeof AssetGet === "function") ? AssetGet(family, b.Group, b.Name) : null;
		if (!A) { skipBlocked(b, "本地无此部件"); continue; }   
		let item = null;
		try {
			if (typeof CharacterAppearanceSetItem === "function") {
				item = CharacterAppearanceSetItem(C, b.Group, A, b.Color, 0);
			} else if (typeof ServerBundledItemToAppearanceItem === "function") {
				
				item = ServerBundledItemToAppearanceItem(family, b);
				if (item) {
					if (!Array.isArray(C.Appearance)) C.Appearance = [];
					const idx = C.Appearance.findIndex((x) => x && x.Asset && x.Asset.Group && x.Asset.Group.Name === b.Group);
					if (idx >= 0) C.Appearance[idx] = item; else C.Appearance.push(item);
				}
			}
		} catch (e) { PSErr("PSApplyOutfit(set)", e); item = null; }
		if (!item) { skipBlocked(b, "应用失败"); continue; }
		if (typeof b.Difficulty === "number") item.Difficulty = b.Difficulty;
		if (b.Craft != null) item.Craft = b.Craft;
		if (b.Property && typeof b.Property === "object") {
			if (!item.Property || typeof item.Property !== "object") item.Property = {};
			for (const k of Object.keys(b.Property)) item.Property[k] = b.Property[k];
		}
		applied++;
	}
	if (!applied && !clearOnly) return "fail";
	
	
	
	if (Array.isArray(C.Appearance)) {
		const classes = new Set();
		const codeGroups = new Set();
		for (const b of bundle) {
			if (b && typeof b.Group === "string") {
				codeGroups.add(b.Group);
				classes.add(PSOutfitClassOf(b, family));
			}
		}
		const clearClothes = classes.has("clothes");
		const clearBody = classes.has("body");
		const clearCos = classes.has("cos");
		
		
		const clearScope = (opts.selGroups && Array.isArray(opts.selGroups)) ? opts.selGroups : null;
		const clearItems = opts.itemsMode === "override" && (classes.has("items") || (clearScope && clearScope.length > 0));
		PSLastOutfitCleared = [];
		if (clearClothes || clearBody || clearCos || clearItems) {
			for (let i = C.Appearance.length - 1; i >= 0; i--) {
				const it = C.Appearance[i];
				if (!it || !it.Asset || !it.Asset.Group) continue;
				const g = it.Asset.Group;
				if (codeGroups.has(g.Name)) continue;      
				if (g.AllowNone === false) continue;       
				const cls = PSOutfitClassOf({ Group: g.Name }, family);
				let shouldClear = false;
				if (cls === "clothes" && clearClothes) shouldClear = true;
				else if (cls === "body" && clearBody) shouldClear = true;
				else if (cls === "cos" && clearCos) shouldClear = true;
				else if (cls === "items" && clearItems) {
					if (clearScope && !clearScope.includes(g.Name)) continue;   
					shouldClear = true;
				}
				...
				if (!fromSelf) {
					...
					if (locked) {
						removable = false;
					} else if (params && typeof ValidationCanRemoveItem === "function") {
						try { removable = !!ValidationCanRemoveItem(it, params, false); } catch (e) {   }
					}
				}
				if (!removable) continue;
				try {
					if (typeof CharacterAppearanceSetItem === "function") {
						CharacterAppearanceSetItem(C, g.Name, null);
					} else {
						C.Appearance.splice(i, 1);
					}
					PSLastOutfitCleared.push(g.Name);
				} catch (e) {   }
			}
		}
	}
	if (!applied && !PSLastOutfitCleared.length) return "fail";   
	
	
	try {
		if (typeof CharacterRefresh === "function") {
			try { CharacterRefresh(C, true); } catch (e) {   }
		}
	} catch (e) {   }
	try {
	...
	return blocked > 0 ? "partial" : "full";
}

 

 
function PSOutfitPrepareBundle(node, bundle, family) {
	const flags = PSOutfitFlagsFromNode(node);
	const locks = !!(node && node.outfitLocks);
	const selGroups = Array.isArray(node && node.outfitGroups) ? node.outfitGroups : null;
	const out = [];
	for (const b of bundle) {
		const cls = PSOutfitClassOf(b, family);
		if (!PSOutfitClassAllowed(cls, flags)) continue;
		
		
		if (selGroups && cls === "items" && !selGroups.includes(b.Group)) continue;
		const nb = PSDeepClone(b);
		if (nb.Property && typeof nb.Property === "object") {
			if (!locks) {
				
				delete nb.Property.Lock;
				delete nb.Property.LockedBy;
				delete nb.Property.LockMemberNumber;
			} else if (nb.Property.Lock || nb.Property.LockedBy) {
				
				nb.Property.LockMemberNumber = (typeof Player !== "undefined" && Player && typeof Player.MemberNumber === "number") ? Player.MemberNumber : null;
			}
		}
		out.push(nb);
	}
...
function PSSendOutfit(node) {
	const code = PSNormalizeCode(node && node.text);
	if (!code) return false;
	if (!PSCanSend()) return false;
	const bundle = PSDecodeOutfitCode(code);
	if (!bundle) { PSToast(PST("outfitBad")); return false; }
	let C = null;
	if (node && node.outfitTarget === "self") {
		C = (typeof Player !== "undefined" && Player) ? Player : null;
	} else {
		const num = PSActive ? PSActive.targetNum : null;
		C = PSCharObjectOf(num);
		if (!C) C = (typeof Player !== "undefined" && Player) ? Player : null;
	}
	if (!C) return false;
	
	if (typeof Player === "undefined" || !Player || C !== Player) {
		if (typeof C.MemberNumber === "number" && !PSTargetInRoom(C.MemberNumber)) {
			PSToast(PST("outfitTargetGone"));
			return false;
		}
	}
	const family = (typeof C.AssetFamily === "string" && C.AssetFamily) ? C.AssetFamily : "Female3DCG";
	const filtered = PSOutfitPrepareBundle(node, bundle, family);
	const selGroups = (node && Array.isArray(node.outfitGroups)) ? node.outfitGroups : null;
	
	const clearOnly = (node && node.outfitItemsMode === "override") && !!selGroups && selGroups.length > 0;
	if (!filtered.length && !clearOnly) { PSToast(PST("outfitNone")); return false; }
	
	const noneSkipped = bundle.filter((b) => b && PSOutfitClassOf(b, family) === "none");
	const r = PSApplyOutfit(C, filtered, {
		itemsMode: (node && node.outfitItemsMode === "override") ? "override" : "add",
		selGroups,
	});
	if (r === "fail") {
		if (PSLastOutfitBlocked.length) {
			const list = PSLastOutfitBlocked.slice(0, 6).join("、") + (PSLastOutfitBlocked.length > 6 ? "…" : "");
			PSToast(PST("outfitBlockedList", list));
		} else {
			PSToast(PST("outfitBlocked"));
		}
		return false;
	}
	if (r === "partial") {
		const list = PSLastOutfitBlocked.slice(0, 6).join("、") + (PSLastOutfitBlocked.length > 6 ? "…" : "");
		PSToast(PST("outfitPartial", list));
		return true;
	}
	if (noneSkipped.length > 0) {
		const list = noneSkipped.slice(0, 6).map((b) => b.Group + "/" + b.Name).join("、") + (noneSkipped.length > 6 ? "…" : "");
		PSToast(PST("outfitAppliedSkip", PSCharDisplayName(C) || "", noneSkipped.length, list));
	} else if (PSLastOutfitCleared.length > 0) {
		PSToast(PST("outfitAppliedClear", PSCharDisplayName(C) || "", PSLastOutfitCleared.length));
	} else {
		PSToast(PST("outfitApplied", PSCharDisplayName(C) || ""));
	}
	return true;
}

 
function PSSendNode(node) {
	if (!PSCanSend()) return "offline";
	const fn = PSSendByType[node.type];
...
function PSFire(scriptId, opts) {
	...
	let triggerNum = opts.senderNum;
	if (triggerNum === undefined) {
		triggerNum = (typeof Player !== "undefined" && Player && typeof Player.MemberNumber === "number") ? Player.MemberNumber : null;
	}
	let targetNum = opts.targetNum;
	if (targetNum === undefined) targetNum = triggerNum;   

	
	if (!opts.manual) {
		const cd = Number(s.cooldown) || 0;
		if (cd > 0) {
			const wait = ((PSLastFire[s.id] || 0) + cd * 1000) - PSNow();
			if (wait > 0) { PSToast(PST("toastCooldown", Math.ceil(wait / 1000))); return { ok: false, why: "cooldown" }; }
		}
		PSLastFire[s.id] = PSNow();
	}

	
	if (PSActive) {
		const mode = opts.manual ? "restart" : s.onRetrigger;
		if (mode === "ignore") { PSToast(PST("toastBusy")); return { ok: false, why: "busy" }; }
		if (mode === "queue") {
...
function PSRun(s, nodes, triggerNum, targetNum) {
	PSActive = {
		scriptId: s.id,
		name: s.name,
		nodes: nodes.map((n) => ({
			id: n.id, type: n.type,
			text: (n.type === "outfit" ? PSNormalizeCode(n.text) : PSNormalizeText(n.text)),
			delay: n.delay, force: !!n.force,
			outfitTarget: n.outfitTarget === "self" ? "self" : "target",
			outfitClothes: n.outfitClothes !== false,
			outfitBody: n.outfitBody !== false,
			outfitItems: n.outfitItems !== false,
...
function PSAdvance() {
	...
	A.timer = setTimeout(() => {
		A.timer = null;
		const r = PSSendNode(next);
		if (r === "offline") { PSAbortCore(false, "noroom"); return; }
...
function PSFinish() {
	...
	if (A) {
		PSLog("演出结束：", A.name);
		PSToast(PST("toastFinished", A.name));
	}
	if (PSQueue.length) {
		const next = PSQueue.shift();
		setTimeout(() => {
			const s = PSFindScript(next.id);
			if (!s) return;
			const nodes = s.nodes.filter((n) => n.enabled !== false && (n.type === "leave" || PSNormalizeText(n.text)));
			if (nodes.length) PSRun(s, nodes, next.triggerNum, next.targetNum);
		}, 300);
	}
	PSUIRenderStatus();
}
...
function PSAbortCore(silent, reason) {
	const A = PSActive;
	if (!A) return false;
	if (A.timer) { clearTimeout(A.timer); A.timer = null; }
	PSActive = null;
	if (!silent) {
		if (reason === "noroom") PSToast(PST("toastAbortRoom"));
		else PSToast(PST("toastAborted", A.name));
	}
	PSLog("演出中止：", A.name, reason || "manual");
	PSUIRenderStatus();
	return true;
}
...
function PSStop() {
	if (!PSActive && !PSQueue.length) { PSToast(PST("toastNothing")); return false; }
	PSQueue = [];
	return PSAbortCore(false, "manual");
}

 
function PSScriptUsesTarget(sc) {
	if (!sc || !Array.isArray(sc.nodes)) return false;
	return sc.nodes.some((n) => {
		if (n && n.type === "outfit") return n.outfitTarget !== "self";
		if (n && typeof n.text === "string") return n.text.includes(PS_TARGET_TOKEN);
		return false;
	});
}

 
function PSTriggerFromText(text, senderNum) {
	if (!PSStore.state) return;
	let me = null;
	if (typeof Player !== "undefined" && Player && typeof Player.MemberNumber === "number") me = Player.MemberNumber;
	const fromMe = (senderNum === null || senderNum === undefined) ? true : (me !== null && senderNum === me);
	for (const sc of PSStore.state.scripts) {
		if (!sc.keyword) continue;
		const who = sc.triggerWho;
		if (who === "self" && !fromMe) continue;
		if (who === "others" && fromMe) continue;
		const info = PSMatchInfo(text, sc);
		if (!info) continue;
		const target = PSTriggerTargetOf(text, sc, info);
		const attempt = PSTriggerTargetAttempt(text, sc, info);
		if (attempt && !target && PSScriptUsesTarget(sc)) {
			
			PSToast(PST("triggerNoTarget", attempt));
			PSLog("目标未找到：", attempt);
			continue;
		}
		PSLog("触发命中：", sc.name, "←", String(text).slice(0, 80), target ? "目标：" + target.name : "");
		PSFire(sc.id, { senderNum, targetNum: target ? target.num : undefined });
	}
}
...
const PS_ACTION_GROUPS = [
	{ name: "any", zh: "任意部位", en: "Any part" },
	{ name: "ItemHead", zh: "头部", en: "Head" },
	{ name: "ItemMouth", zh: "嘴部", en: "Mouth" },
	{ name: "ItemNeck", zh: "颈部", en: "Neck" },
	{ name: "ItemTorso", zh: "躯干", en: "Torso" },
	{ name: "ItemArms", zh: "手臂", en: "Arms" },
	{ name: "ItemHands", zh: "手部", en: "Hands" },
	{ name: "ItemLegs", zh: "腿部", en: "Legs" },
	{ name: "ItemFeet", zh: "脚部", en: "Feet" },
	{ name: "ItemBoots", zh: "鞋子", en: "Boots" },
	{ name: "ItemButt", zh: "臀部", en: "Butt" },
	{ name: "ItemVulva", zh: "私处", en: "Crotch" },
	{ name: "ItemPenis", zh: "私处", en: "Crotch" },
	{ name: "ItemNipples", zh: "胸部", en: "Chest" },
	{ name: "ItemAddon", zh: "装饰", en: "Add-on" },
];

function PSActGroupLabel(name) {
	const g = PS_ACTION_GROUPS.find((x) => x.name === name);
	if (!g) return name;
	return PSUI && PSUI.lang === "en" ? g.en : g.zh;
}
...
function PSActEntriesFor(C) {
	...
	const gname = C.FocusGroup.Name;
	const gbase = PSActBaseGroup(gname);
	const isSelf = PSCharIsSelf(C);
	for (const sc of PSStore.state.scripts) {
		if (sc.enabled === false) continue;
		if (!sc.actionEnabled) continue;
		const label = String(sc.actionName || "").trim();
		if (!label) continue;
		if (sc.actionGroup && sc.actionGroup !== "any" && PSActBaseGroup(sc.actionGroup) !== gbase) continue;
		
		if (sc.actionTarget === "self" && !isSelf) continue;
		if (sc.actionTarget === "others" && isSelf) continue;
		out.push({
			Activity: { Name: "PlayScriptCustom", MaxProgress: 0 },
			Group: gname,
			__psCustom: true,
			__psScriptId: sc.id,
			__psLabel: label.slice(0, 64),
		});
	}
	return out;
}
...
function PSMain() {
	if (typeof window !== "undefined" && window.PlayScriptInstalled) {
		PSLog("已安装，跳过重复加载");
		return;
	}
	const tryRegister = (tries) => {
		if (typeof bcModSdk === "undefined" || !bcModSdk.registerMod) {
			if (tries > 600) {
				PSErr("等待 bcModSdk 超时（60 秒），mod 未能加载");
				return;
			}
		...
		try {
			...
			try {
				if (typeof Player !== "undefined" && Player && typeof Player.MemberNumber === "number") {
					PSStore.ensureAccount(Player.MemberNumber);
				}
			} catch (e) {   }
			...
			PSLog("PlayScript v" + PS_VERSION + " 已加载。聊天室输入 /ps 开关演出脚本浮窗；控制台可用 PlayScriptOpen() / window.PlayScript");
		} catch (e) {
			PSErr("注册 mod 失败", e);
		}
	};
...
const PSText = {
	zh: {
		...
		chipOther: "其他",
		editorHint: "点画布顶部的 ▶ 触发块设置触发方式；点台词节点卡片编辑该句；点左侧剧本行回到这里",
		triggerTargetHint: "对指定玩家演出：说「关键词，对方名字/数字ID」或反过来（如「午时已到，瓦提娜」「天降正义 安缇妮」「255225，英雄不朽」「Vatina 在这停顿」），<target> 自动指向该玩家（英文昵称不分大小写）。剧本有用到 <target> 或服装节点「对目标角色」时，找不到该玩家 → 不执行并提示；剧本不用目标时照常触发。防误触：关键词前后必须是空格或符号，紧挨其它文字不会触发（「我喜欢苹果因为它好吃」不触发，「我喜欢 苹果，因为它好吃」触发）",
		triggerNoTarget: "找不到目标玩家：房间里没有「{0}」（名字或数字 ID 不匹配），本句未触发",
		matchMode: "匹配方式",
		matchContains: "包含关键词",
		matchExact: "完全相等",
		caseSensitive: "区分大小写",
		...
		retryQueue: "排队",
		enabled: "启用",
		actTrigger: "动作触发（游戏动作面板）",
		actHint: "点击角色身体部位的动作面板里会显示这个自定义动作，点它即触发本剧本（等同于手动触发）",
		actName: "动作名",
		actNamePh: "例如：即兴演出",
		actGroup: "出现部位",
		actPick: "选择部位…",
		actWinTitle: "选择动作出现部位",
		actCurrent: "当前：{0}",
		actAny: "任意部位",
		actDone: "完成",
		actTarget: "作用目标",
		actTargetSelf: "只能对自己（点自己的面板可见）",
		actTargetOthers: "只能对别人（点别人的面板可见）",
		otherTrigger: "其他触发",
		...
		orgasmHint: "自己高潮完成时触发「高潮触发」；被拒绝高潮（=毁坏高潮，如高潮锁「拒绝」模式）时触发「拒绝高潮触发」",
		...
		typeRp: "RP 动作 (*)",
		typeNarr: "旁白 (**)",
		typeAction: "Action 消息",
		typeLeave: "离开房间",
		leaveHint: "这个节点会让你离开当前房间（不发送任何台词）。通常放在剧本最后，之后的台词不会发出。",
		leavePreview: "离开当前房间（不发送台词）",
		forceLeave: "强行离开",
		...
		judgeLte: "≤ 阈值 → 是",
		judgePlayer: "玩家ID判定",
		judgePlayerHint: "判定当前演出的对方数字ID，按 ID 走不同分支；都不匹配走「其他玩家」。多个ID用逗号/空格分隔",
		judgePlayerIdsPh: "如 12345、67890",
		...
		judgeCountNode: "本节点激活次数",
		judgeCountNodeHint: "统计剧本每次演出走到这个判定节点本身的次数；每次经过该节点时计数 +1",
		...
		outfitTargetTarget: "对目标角色（无目标时=自己）",
		outfitPreview: "服装变更",
		outfitBad: "服装代码无效：解析失败，这句已跳过（请确认粘贴了完整代码、无多余换行）",
		outfitBlocked: "服装应用失败：未成功应用任何内容，这句已跳过",
		outfitBlockedList: "服装应用失败：未成功应用任何内容（{0}），这句已跳过",
		outfitPartial: "服装部分被拦截（{0}）：其余已应用并同步",
		outfitApplied: "已对 {0} 应用服装",
		outfitAppliedSkip: "已对 {0} 应用服装（跳过 {1} 件不支持部位：{2}）",
		outfitAppliedClear: "已对 {0} 应用服装（清空 {1} 个代码未提及的部位）",
		outfitItemsModeLabel: "道具模式",
		outfitItemsModeAdd: "添加（只补上缺失的道具，不碰已有）",
		outfitItemsModeOverride: "覆盖（替换并清掉代码里没有的道具）",
		outfitTargetGone: "目标角色已不在房间，服装未应用",
		outfitModeLabel: "应用范围",
		outfitBodyLabel: "身体",
		outfitClothesLabel: "服装",
		outfitItemsLabel: "道具",
		outfitCosLabel: "cos道具",
		outfitLocksLabel: "应用锁",
		outfitLocksHint: "勾选后代码里的锁/束缚属性会应用（对别人时走游戏权限校验，锁归属记为你）；不勾选则只换物品不带锁",
		outfitPickLabel: "道具部位",
		outfitPickBtn: "选择部位…",
		outfitGroupsAll: "全部组",
		outfitGroupsN: "已选 {0} 组",
		outfitWinTitle: "选择要应用的道具部位",
		outfitWinHint: "橙色 = 代码里有道具的部位，绿色 = 要应用的部位，点击切换；添加模式只有代码里有的部位可点，覆盖模式所有格子可点（选中但代码里没有道具的格子会被清空）",
		outfitNone: "按当前设置没有可应用的内容，这句已跳过",
		outfitNeedNode: "请先选中一个服装道具变更节点",
		outfitAll: "全选",
		outfitNoneBtn: "清空",
		nodeDelay: "延时（秒，0=立刻）",
		nodeText: "台词文本",
		...
		timeRuleHint: "从上到下匹配，<time>将会替换成第一个满足条件的备选演出句；没有命中 → 替换为空。时间用 24 小时制；时区可填 local、UTC+8、UTC-5:30，或 Asia/Shanghai、America/New_York 等；支持跨午夜（如 22:00~02:00）",
		timeRuleFrom: "开始时间",
		timeRuleTo: "结束时间",
		timeRuleTz: "时区",
		timeRuleText: "替换台词",
		timeRuleTextPh: "时间判定的备选演出句",
		timeRuleAdd: "添加时间段",
		insAfter: "在下方插入",
		insBefore: "在上方插入",
		moveUp: "上移",
		...
		delNode: "删除",
		delNodeConfirm: "确认删除",
		nodeHint: "点流程里的节点卡片编辑台词；没有台词时点「在下方插入」添加。",
		triggerLabel: "触发",
		delayNow: "立刻",
		delayChip: "{0}秒",
		addNode: "+ 添加台词",
		nodeEmpty: "还没有台词：点「+ 添加台词」写第一句",
		noScript: "还没有剧本：点工具栏「新建剧本」",
		selectNodeFirst: "先在中间流程里点一个节点",
		statusIdle: "待机中：在聊天里说出触发关键词，演出自动开始",
		statusRunning: "▶ 演出中：「{0}」 第 {1}/{2} 句 · 下一句 {3}s",
		statusQueued: "排队中：还有 {0} 个剧本待演",
		importTitle: "导入剧本",
		importHint: "粘贴导出的 JSON 文本，或选择 .txt 文件；与现有剧本合并（同 id 覆盖）",
		...
		cloudExportBtn: "导出",
		cloudImportBtn: "导入",
		cloudImportInvalid: "导入失败：无法解析 MD 备份",
		cloudImportDone: "已恢复",
		...
		toastQueued: "已排队：「{0}」（第 {1} 位）",
		toastQueueFull: "排队已满（8 个），本次触发丢弃",
		toastCooldown: "冷却中：约 {0} 秒后可再次触发",
		toastNeedRoom: "需要先进入聊天室",
		toastNothing: "当前没有正在进行的演出",
		toastExported: "已导出：复制到剪贴板并下载文件",
		toastExportFail: "导出失败（剪贴板不可用）",
		toastImported: "导入完成：新增 {0} 个，覆盖 {1} 个",
		toastImportBad: "导入失败：不是有效的 PlayScript 数据",
		toastCleared: "已清空全部剧本",
		toastNoSelScript: "请先在左侧选择一个剧本",
		toastMaxScripts: "剧本数量已达上限（{0} 个）",
		toastMaxNodes: "台词数量已达上限（{0} 句）",
		toastEmptyText: "这句台词是空的，不会被发送",
		toastEmptyNodes: "「{0}」没有可发送的台词（空台词/已停用）",
		toastKwEmpty: "请先填写触发关键词",
		toastNodeAdded: "已添加台词",
		toastScriptAdded: "已新建剧本",
		toastScriptDeleted: "剧本已删除",
		toastDup: "已复制剧本",
		toastDataLoaded: "已加载账号剧本数据（{0} 个剧本）",
	},
	en: {
		title: "PlayScript Stage",
		...
		langBtn: "中文",
		minimizeTitle: "Minimize (click title bar to restore)",
		closeTitle: "Close window",
		resizeTitle: "Drag to resize",
		editorResizeTitle: "Drag to resize the settings panel",
		scriptSettings: "Script settings",
		scriptName: "Script name",
		keyword: "Trigger keyword",
		keywordPh: "e.g. summon",
		triggerSettings: "Trigger settings",
		chipOther: "Other",
		editorHint: "Click the ▶ trigger block at the top of the canvas to edit triggers; click a line card to edit it; click the script row on the left to come back here",
		triggerTargetHint: "Target a player: say \"keyword, name-or-ID\" or the reverse (e.g. \"午时已到，瓦提娜\", \"天降正义 安缇妮\", \"255225，英雄不朽\", \"Vatina 在这停顿\") and <target> points to that player (names are case-insensitive). If the script uses <target> or an outfit node targeting others and no such player exists, the script won't run and a notice is shown; scripts without targets trigger normally. Anti-misfire: the keyword must be surrounded by spaces or punctuation — touching other characters won't trigger",
		triggerNoTarget: "Target not found: no player \"{0}\" in the room — trigger skipped",
		matchMode: "Match mode",
		matchContains: "Contains",
		matchExact: "Exact",
		caseSensitive: "Case sensitive",
		triggerWho: "Triggered by",
		whoSelf: "Only me",
		whoAnyone: "Anyone",
		whoOthers: "Others",
		cooldown: "Cooldown (sec, 0=off)",
		onRetrigger: "Re-trigger while playing",
		retryIgnore: "Ignore",
		retryRestart: "Restart",
		retryQueue: "Queue",
		enabled: "Enabled",
		actTrigger: "Action trigger (game action panel)",
		actHint: "This custom action appears in the action panel when clicking a body part; clicking it triggers this script (same as manual trigger)",
		actName: "Action name",
		actNamePh: "e.g. Improv show",
		actGroup: "Body part",
		actPick: "Pick part…",
		actWinTitle: "Pick body part",
		actCurrent: "Current: {0}",
		actAny: "Any part",
		actDone: "Done",
		actTarget: "Target",
		actTargetSelf: "Self only (visible on own panel)",
		actTargetOthers: "Others only (visible on others' panels)",
		...
		orgasmHint: "Fires when you orgasm; the deny trigger fires only on a ruined orgasm (e.g. orgasm lock deny mode).",
		...
		typeNarr: "Narration (**)",
		typeAction: "Action",
		typeLeave: "Leave room",
		leaveHint: "This node makes you leave the current room (no message is sent). Usually placed last; following lines will not be sent.",
		leavePreview: "Leaves the current room (no message)",
		...
		judgePlayer: "Player ID",
		judgePlayerHint: "Judge the other player's numeric ID and route to matching branch; non-matches go to Other players. Separate multiple IDs with commas/spaces",
		judgePlayerIdsPh: "e.g. 12345, 67890",
		...
		judgeCountNode: "Node activation count",
		judgeCountNodeHint: "Counts how many times the performance reaches this judge node itself; every pass through this node increments the counter",
		...
		outfitTargetTarget: "Target (falls back to self)",
		outfitPreview: "Outfit",
		outfitBad: "Invalid outfit code: parse failed, line skipped (make sure the full code was pasted, no stray line breaks)",
		outfitBlocked: "Outfit apply failed: nothing was applied, line skipped",
		outfitBlockedList: "Outfit not applied: nothing could be applied ({0}), line skipped",
		outfitPartial: "Outfit partially blocked ({0}): the rest applied and synced",
		outfitApplied: "Outfit applied to {0}",
		outfitAppliedSkip: "Outfit applied to {0} ({1} unsupported part(s) skipped: {2})",
		outfitAppliedClear: "Outfit applied to {0} ({1} slot(s) not in code cleared)",
		outfitItemsModeLabel: "Items mode",
		outfitItemsModeAdd: "Add (only fill empty slots, leave existing)",
		outfitItemsModeOverride: "Override (replace and remove items not in code)",
		outfitTargetGone: "Target has left the room, outfit not applied",
		outfitModeLabel: "Apply scope",
		outfitBodyLabel: "Body",
		outfitClothesLabel: "Clothes",
		outfitItemsLabel: "Items",
		outfitCosLabel: "Cosplay items",
		outfitLocksLabel: "Apply locks",
		outfitLocksHint: "When checked, lock/restraint properties from the code are applied (game permission checks apply for others; lock assigned to you); unchecked = items only, no locks",
		outfitPickLabel: "Item parts",
		outfitPickBtn: "Pick parts…",
		outfitGroupsAll: "All groups",
		outfitGroupsN: "{0} selected",
		outfitWinTitle: "Pick item parts to apply",
		outfitWinHint: "Orange = parts with items in the code, green = parts to apply; click to toggle. Add mode: only code parts are clickable. Override mode: every slot is clickable (a selected slot without a code item will be cleared)",
		outfitNone: "Nothing to apply with the current settings, line skipped",
		outfitNeedNode: "Select an outfit node first",
		outfitAll: "All",
		outfitNoneBtn: "None",
		...
		timeRuleHint: "Matched top to bottom: <time> is replaced with the first matching alternative line; no match → empty. 24-hour times. Timezone can be local, UTC+8, UTC-5:30, or Asia/Shanghai, America/New_York, etc. Cross-midnight ranges (e.g. 22:00~02:00) are supported",
		timeRuleFrom: "From",
		timeRuleTo: "To",
		timeRuleTz: "Timezone",
		timeRuleText: "Replacement text",
		timeRuleTextPh: "Alternative line for this time slot",
		...
		moveDown: "Move down",
		delNode: "Delete",
		delNodeConfirm: "Confirm delete",
		nodeHint: "Click a node card in the flow to edit its line. If empty, use Insert below.",
		triggerLabel: "Trigger",
		delayNow: "now",
		delayChip: "{0}s",
		addNode: "+ Add line",
		nodeEmpty: "No lines yet. Click + Add line to write the first one.",
		noScript: "No scripts yet. Click New script in the toolbar.",
		selectNodeFirst: "Select a node in the flow first",
		statusIdle: "Idle: say a trigger keyword in chat to start the show",
		statusRunning: "▶ Playing: {0} — line {1}/{2}, next in {3}s",
		statusQueued: "Queued: {0} script(s) waiting",
		importTitle: "Import scripts",
		importHint: "Paste exported JSON or choose a .txt file; merges with existing scripts (same id = replace)",
		...
		cloudExportBtn: "Export",
		cloudImportBtn: "Import",
		cloudImportInvalid: "Import failed: cannot parse the MD backup",
		cloudImportDone: "Restored",
		...
		toastQueued: "Queued: {0} (#{1})",
		toastQueueFull: "Queue full (8), trigger dropped",
		toastCooldown: "Cooldown: about {0}s left",
		toastNeedRoom: "Enter a chat room first",
		toastNothing: "No show is running",
		toastExported: "Exported: copied to clipboard and downloaded",
		toastExportFail: "Export failed (clipboard unavailable)",
		toastImported: "Imported: {0} added, {1} replaced",
		toastImportBad: "Import failed: not valid PlayScript data",
		toastCleared: "All scripts cleared",
		toastNoSelScript: "Select a script on the left first",
		toastMaxScripts: "Script limit reached ({0})",
		toastMaxNodes: "Line limit reached ({0})",
		toastEmptyText: "This line is empty and will be skipped",
		toastEmptyNodes: "{0} has no sendable lines (empty/disabled)",
		toastKwEmpty: "Fill in the trigger keyword first",
		toastNodeAdded: "Line added",
		toastScriptAdded: "Script created",
		toastScriptDeleted: "Script deleted",
		toastDup: "Script duplicated",
		toastDataLoaded: "Account data loaded ({0} scripts)",
	},
};

 
function PST(key, ...args) {
	let t = (typeof PSUI !== "undefined" && PSUI && PSUI.lang === "en" && PSText.en && PSText.en[key]) ? PSText.en[key] : PSText.zh[key];
	if (t === undefined) t = String(key);
	if (args.length) {
		for (let i = 0; i < args.length; i++) t = t.replace("{" + i + "}", String(args[i]));
	}
	return t;
}
...
const PSUI = {
	...
	open: false,
	el: null,          
	titleEl: null, toolbarEl: null, bodyEl: null,
	btnCloud: null, cloudCapInp: null,   
	listEl: null, flowEl: null, editorEl: null, statusEl: null, cornerEl: null,
...
function PSStyle(el, obj) {
	for (const k in obj) el.style[k] = obj[k];
}

function PSEl(tag, style, html) {
	const e = document.createElement(tag);
	if (style) PSStyle(e, style);
	if (html !== undefined && html !== null) e.innerHTML = html;
	return e;
}

const PS_ACCENT = "#2ec4b6";
const PS_BG = "#1b1e2b";
const PS_PANEL = "#232838";
const PS_BORDER = "#3a4160";
const PS_TEXT = "#e8e8f0";
const PS_TEXT_DIM = "#9aa0b5";

 
function PSBigBtn(label, onClick, opts) {
	opts = opts || {};
	const b = PSEl("button", {
		padding: "10px 16px",
		fontSize: "15px",
...
function PSSmallBtn(label, onClick, opts) {
	opts = opts || {};
	const b = PSEl("button", {
		padding: "4px 8px",
		fontSize: "12px",
		fontFamily: "sans-serif",
		background: opts.bg || "#2c3350",
		color: "#ffffff",
		border: "1px solid #6a7290",
		borderRadius: "6px",
		cursor: "pointer",
	}, PSEsc(label));
	b.title = opts.title || "";
...
function PSArm(key, el, confirmLabel, action) {
	if (PSUI.armed[key]) {
		const a = PSUI.armed[key];
		clearTimeout(a.t);
		delete PSUI.armed[key];
		action();
		return;
	}
	...
	PSStyle(el, { background: "#e74c3c", borderColor: "#ffb3b3" });
	const t = setTimeout(() => {
		delete PSUI.armed[key];
		el.textContent = origText;
		PSStyle(el, { background: origBg, borderColor: origBorder });
	}, 3000);
	PSUI.armed[key] = { t, el, origText, origBg, origBorder };
}

 

function PSToast(msg) {
	if (typeof document === "undefined" || !document.body) { console.log("[PlayScript]", msg); return; }
	try {
		if (!PSUI.toastEl) {
			PSUI.toastEl = document.createElement("div");
			PSUI.toastEl.id = "ps-toast";
			PSStyle(PSUI.toastEl, {
				position: "fixed", left: "16px", top: "16px", zIndex: "2147483600",
				background: "#22283a", border: "1px solid " + PS_ACCENT, color: "#d7f7f2",
				borderRadius: "8px", padding: "10px 16px", fontSize: "14px",
				fontFamily: "sans-serif", transition: "opacity .4s", display: "none",
				maxWidth: "420px", pointerEvents: "none",
			});
			document.body.appendChild(PSUI.toastEl);
		}
		PSUI.toastEl.textContent = msg;
		PSUI.toastEl.style.display = "block";
		PSUI.toastEl.style.opacity = "1";
		if (PSUI.toastTimer) clearTimeout(PSUI.toastTimer);
		PSUI.toastTimer = setTimeout(() => {
			PSUI.toastTimer = null;
			if (PSUI.toastEl) PSUI.toastEl.style.display = "none";
		}, 2600);
	} catch (e) {   }
}
...
function PSUIDotSave() {
	if (!PSStore.state) return;
	PSStore.state.ui.dotPos = PSUI.dotPos;
	PSStore.requestSave();
}

function PSUIDotBuild() {
	if (PSUI.dot || typeof document === "undefined" || !document.body) return;
	let pos = (PSStore.state && PSStore.state.ui && PSStore.state.ui.dotPos) || null;
	if (!pos || typeof pos.x !== "number" || typeof pos.y !== "number") {
		pos = { x: (window.innerWidth || 1000) - 60, y: (window.innerHeight || 700) - 60 };
	}
	const clamped = PSUIDotClamp(pos.x, pos.y);
	PSUI.dotPos = clamped;

	const dot = document.createElement("div");
	dot.id = "ps-dot";
	PSStyle(dot, {
		position: "fixed", left: clamped.x + "px", top: clamped.y + "px",
		width: "46px", height: "46px", borderRadius: "50%",
		background: PS_ACCENT, color: "#0b2623",
		border: "2px solid #ffffff", boxShadow: "0 4px 14px rgba(0,0,0,.5)",
		display: "flex", alignItems: "center", justifyContent: "center",
		fontSize: "20px", fontWeight: "bold", cursor: "pointer",
		zIndex: "2147483400", userSelect: "none", fontFamily: "sans-serif",
	});
	dot.textContent = "▶";
	dot.title = "PlayScript（点击开关浮窗，按住拖动）";

	dot.addEventListener("mousedown", (ev) => {
		ev.preventDefault && ev.preventDefault();
		PSUI.dotDrag = { sx: ev.clientX, sy: ev.clientY, ox: PSUI.dotPos.x, oy: PSUI.dotPos.y, moved: false };
		const move = (e) => {
			if (!PSUI.dotDrag) return;
			const dx = e.clientX - PSUI.dotDrag.sx, dy = e.clientY - PSUI.dotDrag.sy;
			if (Math.abs(dx) + Math.abs(dy) > 5) PSUI.dotDrag.moved = true;
			const c = PSUIDotClamp(PSUI.dotDrag.ox + dx, PSUI.dotDrag.oy + dy);
			PSUI.dotPos = c;
			PSStyle(dot, { left: c.x + "px", top: c.y + "px" });
		};
		const up = () => {
			document.removeEventListener("mousemove", move);
			document.removeEventListener("mouseup", up);
			if (!PSUI.dotDrag) return;
			const moved = PSUI.dotDrag.moved;
			PSUI.dotDrag = null;
			if (moved) PSUIDotSave();
			else PlayScriptToggle();
		};
...
function PSUIWinClamp() {
	const vw = window.innerWidth || 1000, vh = window.innerHeight || 700;
	PSUI.win.w = PSClamp(PSUI.win.w, PS_MIN_WIN_W, Math.max(PS_MIN_WIN_W, vw - 8));
	PSUI.win.h = PSClamp(PSUI.win.h, PS_MIN_WIN_H, Math.max(PS_MIN_WIN_H, vh - 8));
	PSUI.win.x = PSClamp(PSUI.win.x ?? (vw - PSUI.win.w) / 2, 4, Math.max(4, vw - PSUI.win.w - 4));
	PSUI.win.y = PSClamp(PSUI.win.y ?? (vh - PSUI.win.h) / 2, 4, Math.max(4, vh - PSUI.win.h - 4));
}

function PSUIWinSave() {
	if (!PSStore.state) return;
	PSStore.state.ui.win = { x: PSUI.win.x, y: PSUI.win.y, w: PSUI.win.w, h: PSUI.win.h, minimized: PSUI.win.minimized, editorW: PSUI.win.editorW };
	PSStore.requestSave();
}

function PSUIRoot() {
	if (PSUI.el || typeof document === "undefined" || !document.body) return PSUI.el;
	
	if (PSStore.state && PSStore.state.ui && PSStore.state.ui.win) {
		const g = PSStore.state.ui.win;
		if (typeof g.x === "number" && typeof g.y === "number") PSUI.win.x = g.x;
		if (typeof g.y === "number") PSUI.win.y = g.y;
		if (typeof g.w === "number") PSUI.win.w = g.w;
		if (typeof g.h === "number") PSUI.win.h = g.h;
		PSUI.win.minimized = !!g.minimized;
		if (typeof g.editorW === "number") PSUI.win.editorW = g.editorW;
	}
	PSUIWinClamp();
	...
	el.id = "ps-win";
	PSStyle(el, {
		position: "fixed", left: PSUI.win.x + "px", top: PSUI.win.y + "px",
		width: PSUI.win.w + "px", height: PSUI.win.h + "px",
		zIndex: "2147483500", background: PS_BG, border: "2px solid #ffffff",
		borderRadius: "10px", boxShadow: "0 8px 40px rgba(0,0,0,.6)",
		display: "none", color: PS_TEXT, fontFamily: "sans-serif", fontSize: "14px",
		overflow: "hidden",
	});
	PSUI.el = el;

	 
	const title = PSEl("div", {
		display: "flex", alignItems: "center", gap: "8px",
		padding: "8px 12px", background: "#141826", borderBottom: "1px solid " + PS_BORDER,
		cursor: "move", userSelect: "none",
	});
	title.appendChild(PSEl("span", { color: PS_ACCENT, fontSize: "18px" }, "▶"));
	PSUI.titleText = PSEl("span", { fontWeight: "700", fontSize: "15px", flex: "1" }, PSEsc(PST("title")));
	title.appendChild(PSUI.titleText);
	const minBtn = PSSmallBtn("—", () => PSUIMinimizeToggle(), { title: PST("minimizeTitle") });
	const closeBtn = PSSmallBtn("✕", PlayScriptClose, { title: PST("closeTitle") });
	title.appendChild(minBtn);
	title.appendChild(closeBtn);
	title.addEventListener("mousedown", (ev) => {
		if (ev.target === minBtn || ev.target === closeBtn) return;
		PSUI.drag = { sx: ev.clientX, sy: ev.clientY, ox: PSUI.win.x, oy: PSUI.win.y };
		const move = (e) => {
			if (!PSUI.drag) return;
			PSUI.win.x = PSClamp(PSUI.drag.ox + e.clientX - PSUI.drag.sx, 4, Math.max(4, (window.innerWidth || 1000) - PSUI.win.w - 4));
			PSUI.win.y = PSClamp(PSUI.drag.oy + e.clientY - PSUI.drag.sy, 4, Math.max(4, (window.innerHeight || 700) - PSUI.win.h - 4));
			PSStyle(el, { left: PSUI.win.x + "px", top: PSUI.win.y + "px" });
		};
		const up = () => {
			document.removeEventListener("mousemove", move);
			document.removeEventListener("mouseup", up);
	...
	const bar = PSEl("div", {
		...
		padding: "8px 12px", background: "#20243a", borderBottom: "1px solid " + PS_BORDER,
	});
	PSUI.langBtnEl = PSBigBtn(PST("langBtn"), PSUILangToggle);
	PSUI.btnNew = PSBigBtn(PST("newScript"), PSUINewScript);
	PSUI.btnTest = PSBigBtn(PST("testRun"), PSUITestRun, { bg: "#2c7a70" });
	PSUI.btnStop = PSBigBtn(PST("stop"), PSStop);
	PSUI.btnExport = PSBigBtn(PST("exportBtn"), PSUIExport);
	PSUI.btnImport = PSBigBtn(PST("importBtn"), PSUIImportToggle);
	const clearBtn = PSBigBtn(PST("clearAll"), () => PSArm("clearall", clearBtn, PST("clearAllConfirm"), () => { PSClearAll(); PSUIRenderAll(); }), { bg: "#7a2c3a" });
	PSUI.btnClear = clearBtn;
	bar.appendChild(PSUI.btnNew);
	bar.appendChild(PSUI.btnTest);
	bar.appendChild(PSUI.btnStop);
	bar.appendChild(PSUI.btnExport);
	bar.appendChild(PSUI.btnImport);
	...
	capWrap.appendChild(PSEl("span", {}, PSEsc(PST("cloudCapLabel"))));
	PSUI.cloudCapInp = document.createElement("input");
	PSUI.cloudCapInp.type = "number";
	PSUI.cloudCapInp.min = "0";
	PSUI.cloudCapInp.step = "1";
	PSUI.cloudCapInp.value = String(PSStore.cloudCapKB());
	PSStyle(PSUI.cloudCapInp, {
		width: "56px", padding: "6px 6px", fontSize: "12px", borderRadius: "6px",
		background: "#10141f", color: PS_TEXT, border: "1px solid " + PS_BORDER, fontFamily: "sans-serif",
	});
	PSUI.cloudCapInp.addEventListener("change", () => {
		const n = Number(PSUI.cloudCapInp.value);
		PSStore.state.cloudCapKB = (isFinite(n) && n >= 0) ? n : PS_CLOUD_DEFAULT_CAP_KB;
		PSStore.requestSave();
		PSUIRenderAll();
	});
	capWrap.appendChild(PSUI.cloudCapInp);
	...
	const editorW = PSClamp(Number(PSUI.win.editorW) || 340, 300, 900);
	PSUI.win.editorW = editorW;
	PSUI.editorEl = PSEl("div", { width: editorW + "px", minWidth: editorW + "px", overflowY: "auto", background: "#181c2a", borderLeft: "1px solid " + PS_BORDER, padding: "10px" });
	...
	const editHandle = PSEl("div", { width: "6px", minWidth: "6px", cursor: "ew-resize", background: "#0f1320", borderLeft: "1px solid " + PS_BORDER, borderRight: "1px solid " + PS_BORDER });
	editHandle.title = PST("editorResizeTitle");
	editHandle.addEventListener("mousedown", (ev) => {
		ev.preventDefault && ev.preventDefault();
		ev.stopPropagation && ev.stopPropagation();
		const sx = ev.clientX, sw = PSUI.win.editorW;
		const move = (e) => {
			const w = PSClamp(sw + (sx - e.clientX), 300, 900);
			PSUI.win.editorW = w;
			PSStyle(PSUI.editorEl, { width: w + "px", minWidth: w + "px" });
		};
		const up = () => {
			document.removeEventListener("mousemove", move);
			document.removeEventListener("mouseup", up);
	...
	const corner = PSEl("div", {
		position: "absolute", right: "2px", bottom: "2px", width: "18px", height: "18px",
		cursor: "nwse-resize", zIndex: "6",
		borderRight: "2px solid #8891b8", borderBottom: "2px solid #8891b8",
	});
	corner.title = PST("resizeTitle");
	corner.addEventListener("mousedown", (ev) => {
		ev.preventDefault && ev.preventDefault();
		PSUI.resize = { sx: ev.clientX, sy: ev.clientY, ow: PSUI.win.w, oh: PSUI.win.h };
		const move = (e) => {
			if (!PSUI.resize) return;
			PSUI.win.w = PSClamp(PSUI.resize.ow + e.clientX - PSUI.resize.sx, PS_MIN_WIN_W, Math.max(PS_MIN_WIN_W, (window.innerWidth || 1000) - 8));
			PSUI.win.h = PSClamp(PSUI.resize.oh + e.clientY - PSUI.resize.sy, PS_MIN_WIN_H, Math.max(PS_MIN_WIN_H, (window.innerHeight || 700) - 8));
			PSStyle(el, { width: PSUI.win.w + "px", height: PSUI.win.h + "px" });
		};
		const up = () => {
			document.removeEventListener("mousemove", move);
			document.removeEventListener("mouseup", up);
	...
	PSUI.cornerEl = corner;

	 
	PSUI.statusEl = PSEl("div", {
		position: "absolute", left: "0", right: "0", bottom: "0",
		padding: "8px 12px", background: "#141826", borderTop: "1px solid " + PS_BORDER,
		fontSize: "13px", color: "#9fd8d0",
	});
	el.appendChild(PSUI.statusEl);

	 
	PSUIImportBuild(el);

	 
	if (!PSUI.keyBound && typeof document.addEventListener === "function") {
		PSUI.keyBound = true;
...
function PSUIMinimizeToggle() {
	PSUI.win.minimized = !PSUI.win.minimized;
	applyMin();
	PSUIWinSave();
	function applyMin() {
		if (!PSUI.bodyEl) return;
		PSUI.bodyEl.style.display = PSUI.win.minimized ? "none" : "flex";
		PSUI.toolbarEl.style.display = PSUI.win.minimized ? "none" : "flex";
		PSUI.statusEl.style.display = PSUI.win.minimized ? "none" : "block";
		PSUI.cornerEl.style.display = PSUI.win.minimized ? "none" : "block";
		if (PSUI.el) PSUI.el.style.height = PSUI.win.minimized ? "auto" : PSUI.win.h + "px";
	}
}

 

function PSUIListBuild() {
	const box = PSUI.listEl;
	if (!box) return;
	box.innerHTML = "";
	const head = PSEl("div", { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "4px 4px 8px", borderBottom: "1px solid " + PS_BORDER, marginBottom: "8px" });
	head.appendChild(PSEl("span", { fontWeight: "700", fontSize: "15px" }, PSEsc(PST("scriptsHeader"))));
	head.appendChild(PSEl("span", { color: PS_TEXT_DIM, fontSize: "12px" }, String(PSStore.state.scripts.length)));
	box.appendChild(head);

	if (!PSStore.state.scripts.length) {
	...
	for (const sc of PSStore.state.scripts) {
		...
		const row = PSEl("div", {
			...
			border: selected ? "1px solid " + PS_ACCENT : "1px solid " + PS_BORDER,
			cursor: "pointer",
		});
		row.title = PSEsc(sc.name);
		const cb = document.createElement("input");
		cb.type = "checkbox";
		cb.checked = sc.enabled !== false;
		cb.addEventListener("change", () => { PSUpdateScript(sc.id, { enabled: cb.checked }); PSUIRenderAll(); });
		cb.addEventListener("click", (ev) => { if (ev && ev.stopPropagation) ev.stopPropagation(); });
		row.appendChild(cb);
		if (running) row.appendChild(PSEl("span", { color: PS_ACCENT, fontSize: "12px" }, "▶"));
		const mid = PSEl("div", { flex: "1", minWidth: "0" });
		const nameEl = PSEl("div", { fontWeight: "600", fontSize: "13px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }, PSEsc(sc.name));
...
function PSUINodeIcon(type) {
	const m = PSTypeMeta[type] || PSTypeMeta.chat;
	return m.icon;
}

function PSUINodeColor(type) {
	const m = PSTypeMeta[type] || PSTypeMeta.chat;
	return m.color;
}
...
function PSUIFlowBuild() {
	const box = PSUI.flowEl;
	if (!box) return;
	box.innerHTML = "";
	PSUI.flowCards = [];
	const sc = PSFindScript(PSUI.selScriptId);
	if (!sc) {
		box.appendChild(PSEl("div", { color: PS_TEXT_DIM, padding: "24px 10px", textAlign: "center" }, PSEsc(PST("noScript") + " / " + PST("scriptsHint"))));
		return;
	}

	 
	const trigSelected = !PSUI.selNodeId && PSUI.selTrigger;
	const trig = PSEl("div", {
		display: "flex", alignItems: "center", gap: "8px",
		padding: "10px 12px", borderRadius: "10px", marginBottom: "4px",
		background: trigSelected ? "#2c3452" : "#17302e",
		border: "1px solid " + (trigSelected ? PS_ACCENT : "#3f8f86"),
		cursor: "pointer",
	});
	trig.addEventListener("click", () => {
		PSUI.selNodeId = null;
		PSUI.selTrigger = true;
		PSUIRenderAll();
	});
	trig.appendChild(PSEl("span", { fontSize: "18px", color: PS_ACCENT }, "▶"));
	const trigMid = PSEl("div", { flex: "1", minWidth: "0" });
	trigMid.appendChild(PSEl("div", { fontSize: "11px", color: PS_ACCENT, fontWeight: "700" }, PSEsc(PST("triggerLabel"))));
	const kwPreview = PSEl("div", {
		fontSize: "13px", color: PS_TEXT, marginTop: "3px",
		whiteSpace: "pre-wrap", maxHeight: "38px", overflow: "hidden", wordBreak: "break-word",
	}, PSEsc(sc.keyword ? PSUINodePreview(sc.keyword) : PST("keywordPh")));
	trigMid.appendChild(kwPreview);
	trig.appendChild(trigMid);
	
	const chip = (txt, bg, fg) => PSEl("span", {
		display: "inline-block", padding: "2px 8px", marginLeft: "4px",
		fontSize: "11px", borderRadius: "8px", background: bg, color: fg,
		border: "1px solid " + PS_BORDER,
	}, PSEsc(txt));
	const chipRow = PSEl("div", { display: "flex", gap: "4px", flexWrap: "wrap", justifyContent: "flex-end" });
	chipRow.appendChild(chip(sc.matchMode === "exact" ? PST("matchExact") : PST("matchContains"), "#2a3150", "#c9d2f5"));
	chipRow.appendChild(chip(sc.triggerWho === "others" ? PST("whoOthers") : (sc.triggerWho === "anyone" ? PST("whoAnyone") : PST("whoSelf")), "#2a3150", "#c9d2f5"));
	...
	const makeCard = (n) => {
		...
		const card = PSEl("div", {
			...
			cursor: "pointer",
			opacity: n.enabled === false ? "0.55" : "1",
		});
	...
	const renderPath = (n, lane) => {
		...
		if (n.type === "judge") {
			...
			let branchDefs;
			if (n.judgeType === "dice" && Array.isArray(n.diceBranches) && n.diceBranches.length) {
				branchDefs = n.diceBranches.map((b) => ({ label: b.from + "~" + b.to, nodeId: b.nodeId, color: "#4e7fb0" }));
			} else if (n.judgeType === "player") {
				const pbs = Array.isArray(n.playerBranches) ? n.playerBranches : [];
				branchDefs = pbs.map((b, i) => ({ label: (b.ids && b.ids.length) ? b.ids.map((x) => "#" + x).join("/") : PST("judgePlayerBranchPh", i + 1), nodeId: b.nodeId, color: "#b07f2c" }));
				branchDefs.push({ label: PST("judgePlayerElse"), nodeId: n.elseId, color: "#7a2c3a" });
...
function PSUIJudgePreview(n) {
	...
	if (n && n.judgeType === "count") {
		...
		return (n.countMode === "interact" ? "互动次数：" : (n.countMode === "orgasm" ? "高潮次数：" : (n.countMode === "node" ? "本节点激活次数：" : "剧本触发次数："))) + (parts.length ? parts.join("、") : "（未设置分支）");
	}
	...
	if (n && n.judgeType === "player") {
		...
		const parts = pbs.map((b, i) => (b.ids && b.ids.length) ? b.ids.map((x) => "#" + x).join("/") : PST("judgePlayerBranchPh", i + 1));
		return "玩家ID：" + (parts.length ? parts.join("、") : "（未设置分支）") + (n.elseId ? " · " + PST("judgePlayerElse") : "");
	}
...
function PSUINodeDragStart(scriptId, nodeId, fromIdx) {
	if (typeof document.addEventListener !== "function") return;
	PSUI.nodeDrag = { scriptId, nodeId, fromIdx, targetId: null };
	
	const cardEl = PSUI.flowCards.find((c) => c.id === nodeId);
	if (cardEl) cardEl.el.style.opacity = "0.4";
	const move = (e) => {
		if (!PSUI.nodeDrag) return;
		let overId = null;
		try {
			if (typeof document.elementFromPoint === "function") {
				const el = document.elementFromPoint(e.clientX, e.clientY);
				const card = el && typeof el.closest === "function" ? el.closest("[data-node-id]") : null;
				if (card) overId = card.getAttribute("data-node-id");
			}
		} catch (err) {   }
		
		PSUI.flowCards.forEach((c) => { c.el.style.outline = c.id === overId ? "2px dashed " + PS_ACCENT : ""; });
		PSUI.nodeDrag.targetId = overId;
	};
	const up = () => {
		document.removeEventListener("mousemove", move);
		document.removeEventListener("mouseup", up);
		const d = PSUI.nodeDrag;
		PSUI.nodeDrag = null;
		PSUI.flowCards.forEach((c) => { c.el.style.outline = ""; c.el.style.opacity = "1"; });
		if (d && d.targetId && d.targetId !== d.nodeId) {
			const sc = PSFindScript(d.scriptId);
			if (sc) {
				const toIdx = sc.nodes.findIndex((n) => n.id === d.targetId);
				if (toIdx >= 0 && PSMoveNodeTo(sc, d.nodeId, toIdx)) PSUIRenderAll();
			}
		}
	};
	document.addEventListener("mousemove", move);
...
const PS_ACT_ALIASES = { "ItemHandheld": "ItemHands" };

 
function PSActBaseGroup(name) {
	const n = String(name || "");
	if (PS_ACT_ALIASES[n]) return PS_ACT_ALIASES[n];
	return n.replace(/\d+$/, "");
}
...
function PSActZones() {
	const out = [];
	const byName = {};
	if (typeof AssetGroup === "undefined" || !Array.isArray(AssetGroup)) return out;
	for (const g of AssetGroup) {
		try { if (!g || typeof g.IsItem !== "function" || !g.IsItem()) continue; } catch (e) { continue; }
		if (!g.Name || !Array.isArray(g.Zone) || !g.Zone.length) continue;
		const base = PSActBaseGroup(g.Name);
		if (!byName[base]) {
			byName[base] = { name: base, zones: [], hitZones: [] };
			out.push(byName[base]);
		}
		if (g.Name === base) {
			
			for (const z of g.Zone) byName[base].zones.push(z);
		}
		
		for (const z of g.Zone) byName[base].hitZones.push(z);
	}
	return out.filter((e) => e.hitZones.length);
}

 
function PSActCanvasMap(cw, ch) {
	const s = Math.min(cw / 500, ch / 1000);
	return { s, ox: (cw - 500 * s) / 2, oy: (ch - 1000 * s) / 2 };
}

 
function PSActZoneHitTest(x, y, cw, ch) {
	const { s, ox, oy } = PSActCanvasMap(cw, ch);
	const ix = (x - ox) / s, iy = (y - oy) / s;
	for (const g of PSActZones()) {
		for (const z of g.hitZones) {
			if (ix >= z[0] && ix <= z[0] + z[2] && iy >= z[1] && iy <= z[1] + z[3]) return g.name;
		}
	}
	return null;
}
...
function PSActZonesAll() {
	const out = [];
	if (typeof AssetGroup === "undefined" || !Array.isArray(AssetGroup)) return out;
	for (const g of AssetGroup) {
		try { if (!g || typeof g.IsItem !== "function" || !g.IsItem()) continue; } catch (e) { continue; }
		if (!g.Name || !Array.isArray(g.Zone) || !g.Zone.length) continue;
		out.push({ name: g.Name, zones: g.Zone.slice() });
	}
	return out;
}

 
function PSActZoneHitTestAll(x, y, cw, ch) {
	const { s, ox, oy } = PSActCanvasMap(cw, ch);
	const ix = (x - ox) / s, iy = (y - oy) / s;
	for (const g of PSActZonesAll()) {
...
function PSPreviewDrawChar(canvas, bundle) {
	...
	let temp = null;
	try {
		if (typeof CharacterCreate === "function" && typeof CharacterRefresh === "function" &&
			typeof DrawCharacterSegment === "function") {
			const type = (typeof CharacterType !== "undefined" && CharacterType.SIMPLE) ? CharacterType.SIMPLE : "simple";
			const family = (typeof Player !== "undefined" && Player && typeof Player.AssetFamily === "string" && Player.AssetFamily) ? Player.AssetFamily : "Female3DCG";
			temp = CharacterCreate(family, type, "PlayScriptPreview");
			const items = [];
			for (const b of (Array.isArray(bundle) ? bundle : [])) {
			...
			if (seg && seg.width) {
				g.drawImage(seg, ox, oy, 500 * s, 1000 * s);
				drawn = true;
			}
		}
	} catch (e) { PSErr("预览绘制失败", e); }
	finally {
...
function PSPreviewDrawZones(canvas, colorOf, zonesList) {
	const g = canvas.getContext ? canvas.getContext("2d") : null;
	if (!g || typeof colorOf !== "function") return;
	const cw = canvas.width, ch = canvas.height;
	const { s, ox, oy } = PSActCanvasMap(cw, ch);
	for (const grp of (zonesList || PSActZones())) {
		const c = colorOf(grp.name);
		if (!c) continue;
		for (const z of grp.zones) {
			const rx = ox + z[0] * s, ry = oy + z[1] * s, rw = z[2] * s, rh = z[3] * s;
			if (c.fill) { g.fillStyle = c.fill; g.fillRect(rx, ry, rw, rh); }
...
function PSUIActSelectedGroup() {
	const sc = PSFindScript(PSUI.actScriptId);
	return (sc && sc.actionGroup) ? sc.actionGroup : "any";
}

function PSUIActPreviewDraw() {
	const canvas = PSUI.actCanvas;
	if (!canvas || !PSUI.actOpen) return;
	
	let bundle = [];
	try {
		if (typeof Player !== "undefined" && Player && typeof ServerAppearanceBundle === "function") {
			bundle = ServerAppearanceBundle(Player) || [];
		}
	} catch (e) {   }
	PSPreviewDrawChar(canvas, bundle);
	
	
	const sel = PSUIActSelectedGroup();
	PSPreviewDrawZones(canvas, (name) => {
		if (name === sel) return { fill: "rgba(0,255,255,0.25)", stroke: "cyan", width: 2.5 };
		if (name === PSUI.actHover) return { fill: "rgba(255,255,255,0.18)", stroke: "rgba(255,255,255,0.85)", width: 2 };
		return { fill: "#80808040", stroke: "#808080", width: 1.5 };
	});
}

function PSUIActWinBuild() {
	if (PSUI.actWin || typeof document === "undefined" || !document.body) return;
	const win = document.createElement("div");
	win.id = "ps-actwin";
	PSStyle(win, {
		position: "fixed", left: "50%", top: "50%", transform: "translate(-50%, -50%)",
		width: "380px", zIndex: "2147483550", background: PS_BG,
	...
	const title = PSEl("div", {
		...
		background: "#141826", borderBottom: "1px solid " + PS_BORDER, userSelect: "none",
	});
	title.appendChild(PSEl("span", { color: PS_ACCENT, fontSize: "16px" }, "▶"));
	PSUI.actTitleEl = PSEl("span", { fontWeight: "700", fontSize: "14px", flex: "1" });
	title.appendChild(PSUI.actTitleEl);
	const closeBtn = PSSmallBtn("✕", PSUIActWinClose, { title: PST("closeTitle") });
	title.appendChild(closeBtn);
	...
	PSStyle(canvas, {
		display: "block", position: "relative", width: "300px", height: "500px",
		margin: "8px auto 0", cursor: "crosshair",
	});
	canvas.addEventListener("click", (ev) => {
		try {
			...
			if (hit) PSUIActWinSelect(hit);
		} catch (e) { PSErr("部位选择点击失败", e); }
	});
	canvas.addEventListener("mousemove", (ev) => {
		try {
			...
			const y = (ev.clientY - rect.top) * (canvas.height / Math.max(1, rect.height));
			const hit = PSActZoneHitTest(x, y, canvas.width, canvas.height);
			if (hit !== PSUI.actHover) { PSUI.actHover = hit; PSUIActPreviewDraw(); }
		} catch (e) {   }
	});
	canvas.addEventListener("mouseleave", () => {
		if (PSUI.actHover !== null) { PSUI.actHover = null; PSUIActPreviewDraw(); }
	});
	win.appendChild(canvas);

	const foot = PSEl("div", { display: "flex", alignItems: "center", gap: "8px", padding: "10px 12px" });
	const anyBtn = PSBigBtn(PST("actAny"), () => PSUIActWinSelect("any"));
	foot.appendChild(anyBtn);
	PSUI.actChipEl = PSEl("span", {
		flex: "1", textAlign: "center", padding: "8px 10px", borderRadius: "6px",
		background: "#10141f", border: "1px solid " + PS_BORDER, color: PS_ACCENT, fontSize: "13px",
	});
	foot.appendChild(PSUI.actChipEl);
	const doneBtn = PSBigBtn(PST("actDone"), PSUIActWinClose, { bg: "#2c7a70" });
	foot.appendChild(doneBtn);
	win.appendChild(foot);

	document.body.appendChild(win);
	PSUI.actWin = win;
	PSUI.actCanvas = canvas;
}

function PSUIActWinOpen() {
	const sc = PSFindScript(PSUI.selScriptId);
	if (!sc) { PSToast(PST("toastNoSelScript")); return; }
	PSUI.actScriptId = sc.id;
	PSUI.actHover = null;
	PSUIActWinBuild();
	if (!PSUI.actWin) return;
	PSUI.actOpen = true;
	PSUI.actWin.style.display = "block";
	PSUIActWinRender();
}

function PSUIActWinClose() {
	PSUI.actOpen = false;
	PSUI.actHover = null;
	if (PSUI.actWin) PSUI.actWin.style.display = "none";
}

function PSUIActWinSelect(group) {
	const sc = PSFindScript(PSUI.actScriptId);
	if (sc) PSUpdateScript(sc.id, { actionGroup: group });
	PSUIActWinRender();
	PSUIRenderAll();   
}

function PSUIActWinRender() {
	if (!PSUI.actWin) return;
	const sc = PSFindScript(PSUI.actScriptId);
	if (PSUI.actTitleEl) PSUI.actTitleEl.textContent = PST("actWinTitle") + " · " + (sc ? sc.name : "");
	if (PSUI.actChipEl) PSUI.actChipEl.textContent = PST("actCurrent", PSActGroupLabel(PSUIActSelectedGroup()));
	PSUIActPreviewDraw();
}
...
function PSUIOutfitWinOpen() {
	const sc = PSFindScript(PSUI.selScriptId);
	const node = sc ? sc.nodes.find((n) => n.id === PSUI.selNodeId) : null;
	if (!node || node.type !== "outfit") { PSToast(PST("outfitNeedNode")); return; }
	PSUI.outfitScriptId = sc.id;
	PSUI.outfitNodeId = node.id;
	
	const bundle = PSDecodeOutfitCode(PSNormalizeCode(node.text)) || [];
	const family = (typeof Player !== "undefined" && Player && typeof Player.AssetFamily === "string" && Player.AssetFamily) ? Player.AssetFamily : "Female3DCG";
	const codeGroups = [];
	for (const b of bundle) {
		const cls = PSOutfitClassOf(b, family);
		if (cls !== "items" || typeof b.Group !== "string") continue;
		if (!codeGroups.includes(b.Group)) codeGroups.push(b.Group);
	}
	PSUI.outfitCodeGroups = codeGroups;
	PSUI.outfitBundle = bundle;
	
	
	const override = node && node.outfitItemsMode === "override";
	PSUI.outfitPickable = override ? PSActZonesAll().map((z) => z.name) : codeGroups.slice();
	
	const sel = Array.isArray(node.outfitGroups) ? node.outfitGroups.slice() : PSUI.outfitPickable.slice();
	PSUI.outfitSel = sel;
	PSUI.outfitHover = null;
	PSUIOutfitWinBuild();
	if (!PSUI.outfitWin) return;
	PSUI.outfitOpen = true;
	PSUI.outfitWin.style.display = "block";
	PSUIOutfitWinRender();
}

function PSUIOutfitWinClose() {
	PSUI.outfitOpen = false;
	PSUI.outfitHover = null;
	if (PSUI.outfitWin) PSUI.outfitWin.style.display = "none";
}

function PSUIOutfitWinSave() {
	const node = PSUIOutfitNode();
	if (node) PSUpdateNode(PSUI.outfitScriptId, node.id, { outfitGroups: PSUI.outfitSel.slice() });
	PSUIOutfitWinRender();
	PSUIRenderAll();   
}

function PSUIOutfitToggle(name) {
	if (!PSUI.outfitPickable.includes(name)) return;   
	const i = PSUI.outfitSel.indexOf(name);
	if (i >= 0) PSUI.outfitSel.splice(i, 1);
	else PSUI.outfitSel.push(name);
	PSUIOutfitWinSave();
}

function PSUIOutfitAll() {
	PSUI.outfitSel = PSUI.outfitPickable.slice();
	PSUIOutfitWinSave();
}

function PSUIOutfitNone() {
	PSUI.outfitSel = [];
	PSUIOutfitWinSave();
}

function PSUIOutfitWinRender() {
	if (!PSUI.outfitWin) return;
	const node = PSUIOutfitNode();
	if (PSUI.outfitTitleEl) PSUI.outfitTitleEl.textContent = PST("outfitWinTitle") + (node ? "" : "");
	if (PSUI.outfitChipEl) PSUI.outfitChipEl.textContent = PST("outfitGroupsN", PSUI.outfitSel.length, PSUI.outfitPickable.length);
	if (!PSUI.outfitCanvas) return;
	
	PSPreviewDrawChar(PSUI.outfitCanvas, PSUI.outfitBundle);
	
	
	
	PSPreviewDrawZones(PSUI.outfitCanvas, (name) => {
		const hasItem = PSUI.outfitCodeGroups.includes(name);
		const isSel = PSUI.outfitSel.includes(name);
		const isHover = name === PSUI.outfitHover;
		if (isSel) return { fill: "rgba(76,175,80,0.35)", stroke: "#66bb6a", width: 2.5 };
		if (isHover) return { fill: "rgba(255,255,255,0.18)", stroke: "rgba(255,255,255,0.85)", width: 2 };
		if (hasItem) return { fill: "rgba(255,152,0,0.30)", stroke: "#ff9f43", width: 2 };
		return { fill: "#80808040", stroke: "#808080", width: 1.5 };
	}, PSActZonesAll());
}

function PSUIOutfitWinBuild() {
	if (PSUI.outfitWin || typeof document === "undefined" || !document.body) return;
	const win = document.createElement("div");
	win.id = "ps-outfitwin";
	PSStyle(win, {
		position: "fixed", left: "50%", top: "50%", transform: "translate(-50%, -50%)",
		width: "380px", zIndex: "2147483560", background: PS_BG,
		border: "2px solid #ffffff", borderRadius: "10px",
		boxShadow: "0 8px 40px rgba(0,0,0,.7)", display: "none",
		color: PS_TEXT, fontFamily: "sans-serif", fontSize: "14px", overflow: "hidden",
	});
	...
	title.appendChild(PSEl("span", { color: "#9fd8a0", fontSize: "16px" }, "▶"));
	PSUI.outfitTitleEl = PSEl("span", { fontWeight: "700", fontSize: "14px", flex: "1" });
	title.appendChild(PSUI.outfitTitleEl);
	const closeBtn = PSSmallBtn("✕", PSUIOutfitWinClose, { title: PST("closeTitle") });
	title.appendChild(closeBtn);
	win.appendChild(title);

	const hint = PSEl("div", { padding: "6px 12px", fontSize: "12px", color: PS_TEXT_DIM, borderBottom: "1px solid " + PS_BORDER });
	hint.textContent = PST("outfitWinHint");
	win.appendChild(hint);
	...
	canvas.addEventListener("click", (ev) => {
		try {
			...
			const x = (ev.clientX - rect.left) * (canvas.width / Math.max(1, rect.width));
			const y = (ev.clientY - rect.top) * (canvas.height / Math.max(1, rect.height));
			const hit = PSActZoneHitTestAll(x, y, canvas.width, canvas.height);
			if (hit) PSUIOutfitToggle(hit);
		} catch (e) { PSErr("道具部位选择点击失败", e); }
	});
	canvas.addEventListener("mousemove", (ev) => {
		try {
			const rect = canvas.getBoundingClientRect();
			...
			const y = (ev.clientY - rect.top) * (canvas.height / Math.max(1, rect.height));
			const hit = PSActZoneHitTestAll(x, y, canvas.width, canvas.height);
			if (hit !== PSUI.outfitHover) { PSUI.outfitHover = hit; PSUIOutfitWinRender(); }
		} catch (e) {   }
	});
	canvas.addEventListener("mouseleave", () => {
		if (PSUI.outfitHover !== null) { PSUI.outfitHover = null; PSUIOutfitWinRender(); }
	});
	win.appendChild(canvas);

	const foot = PSEl("div", { display: "flex", alignItems: "center", gap: "8px", padding: "10px 12px" });
	foot.appendChild(PSBigBtn(PST("outfitAll"), PSUIOutfitAll));
	foot.appendChild(PSBigBtn(PST("outfitNoneBtn"), PSUIOutfitNone));
	PSUI.outfitChipEl = PSEl("span", {
		flex: "1", textAlign: "center", padding: "8px 10px", borderRadius: "6px",
		background: "#10141f", border: "1px solid " + PS_BORDER, color: "#9fd8a0", fontSize: "13px",
	});
	foot.appendChild(PSUI.outfitChipEl);
	foot.appendChild(PSBigBtn(PST("actDone"), PSUIOutfitWinClose, { bg: "#2c7a70" }));
	win.appendChild(foot);

	document.body.appendChild(win);
	PSUI.outfitWin = win;
	PSUI.outfitCanvas = canvas;
}

 

function PSUIEditorRow(labelText, control) {
...
function PSUIInput(value, onchange, opts) {
	opts = opts || {};
...
function PSUISelect(value, optionsHTML, onchange) {
	const sel = PSEl("select", {
		flex: "1", minWidth: "0", padding: "8px 6px", fontSize: "13px", borderRadius: "6px",
		background: "#10141f", color: PS_TEXT, border: "1px solid " + PS_BORDER, fontFamily: "sans-serif",
	}, optionsHTML);
	sel.value = value;
...
function PSUIEditorBuild() {
	const box = PSUI.editorEl;
	if (!box) return;
	box.innerHTML = "";
	PSUI.textareaEl = null;
	const sc = PSFindScript(PSUI.selScriptId);
	if (!sc) {
		box.appendChild(PSEl("div", { color: PS_TEXT_DIM, padding: "20px 10px", textAlign: "center" }, PSEsc(PST("scriptsHint"))));
		return;
	}

	 
	const node = sc.nodes.find((n) => n.id === PSUI.selNodeId) || null;
	if (node) {
		PSUINodeEditor(box, sc, node);
		return;
	}

	 
	if (PSUI.selTrigger) {
		PSUITriggerSection(box, sc);
		return;
	}

	 
	const sec1 = PSEl("div", { padding: "10px", borderRadius: "8px", background: PS_PANEL, marginBottom: "10px" });
	sec1.appendChild(PSEl("div", { fontWeight: "700", marginBottom: "8px", color: PS_ACCENT }, PSEsc(PST("scriptSettings"))));
	const nameInp = PSUIInput(sc.name, (v) => { PSUpdateScript(sc.id, { name: v }); PSUIRenderAll(); });
	sec1.appendChild(PSUIEditorRow(PST("scriptName"), nameInp));
	
	const bioRow = PSEl("div", { display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" });
	const bioLab = PSEl("label", { width: "110px", minWidth: "110px", fontSize: "13px", color: PS_TEXT_DIM });
	bioLab.textContent = PST("cloudBioLabel");
	bioRow.appendChild(bioLab);
	const bioCb = document.createElement("input");
	bioCb.type = "checkbox";
	bioCb.checked = !!sc.cloudBio;
	bioCb.addEventListener("change", () => { PSUpdateScript(sc.id, { cloudBio: bioCb.checked }); PSUIRenderAll(); });
	bioRow.appendChild(bioCb);
	sec1.appendChild(bioRow);
	const bioHint = PSEl("div", { fontSize: "12px", color: PS_TEXT_DIM, marginBottom: "8px" });
	...
	box.appendChild(PSEl("div", { color: PS_TEXT_DIM, padding: "14px 10px", textAlign: "center" }, PSEsc(PST("editorHint"))));
}

 
function PSUITriggerSection(box, sc) {
	const sec1 = PSEl("div", { padding: "10px", borderRadius: "8px", background: PS_PANEL, marginBottom: "10px" });
	sec1.appendChild(PSEl("div", { fontWeight: "700", marginBottom: "8px", color: PS_ACCENT }, PSEsc(PST("triggerLabel")) + " · " + PSEsc(PST("triggerSettings"))));

	const kwInp = PSUIInput(sc.keyword, (v) => { PSUpdateScript(sc.id, { keyword: v }); PSUIRenderAll(); }, { placeholder: PST("keywordPh") });
	sec1.appendChild(PSUIEditorRow(PST("keyword"), kwInp));

	const matchSel = PSUISelect(sc.matchMode, [
		'<option value="contains">' + PSEsc(PST("matchContains")) + "</option>",
		'<option value="exact">' + PSEsc(PST("matchExact")) + "</option>",
	].join(""), (v) => { PSUpdateScript(sc.id, { matchMode: v }); PSUIRenderAll(); });
	sec1.appendChild(PSUIEditorRow(PST("matchMode"), matchSel));

	const whoSel = PSUISelect(sc.triggerWho, [
		'<option value="self">' + PSEsc(PST("whoSelf")) + "</option>",
		'<option value="anyone">' + PSEsc(PST("whoAnyone")) + "</option>",
		'<option value="others">' + PSEsc(PST("whoOthers")) + "</option>",
	].join(""), (v) => { PSUpdateScript(sc.id, { triggerWho: v }); PSUIRenderAll(); });
	sec1.appendChild(PSUIEditorRow(PST("triggerWho"), whoSel));

	const caseCb = document.createElement("input");
	caseCb.type = "checkbox";
	caseCb.checked = !!sc.caseSensitive;
	caseCb.addEventListener("change", () => { PSUpdateScript(sc.id, { caseSensitive: caseCb.checked }); PSUIRenderAll(); });
	const caseRow = PSEl("div", { display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" });
	const caseLab = PSEl("label", { width: "110px", minWidth: "110px", fontSize: "13px", color: PS_TEXT_DIM });
	caseLab.textContent = PST("caseSensitive");
	caseRow.appendChild(caseLab);
	caseRow.appendChild(caseCb);
	sec1.appendChild(caseRow);

	const cdInp = PSUIInput(String(sc.cooldown), (v) => { PSUpdateScript(sc.id, { cooldown: v }); PSUIRenderAll(); }, { type: "number", min: "0", max: String(PS_MAX_DELAY), step: "1" });
	sec1.appendChild(PSUIEditorRow(PST("cooldown"), cdInp));

	const retrySel = PSUISelect(sc.onRetrigger, [
		'<option value="ignore">' + PSEsc(PST("retryIgnore")) + "</option>",
		'<option value="restart">' + PSEsc(PST("retryRestart")) + "</option>",
		'<option value="queue">' + PSEsc(PST("retryQueue")) + "</option>",
	].join(""), (v) => { PSUpdateScript(sc.id, { onRetrigger: v }); PSUIRenderAll(); });
	sec1.appendChild(PSUIEditorRow(PST("onRetrigger"), retrySel));

	
	const targetHint = PSEl("div", { fontSize: "12px", color: PS_TEXT_DIM, margin: "6px 0 8px", lineHeight: "1.5" });
	targetHint.textContent = PST("triggerTargetHint");
	sec1.appendChild(targetHint);

	 
	const actSec = PSEl("div", {
		padding: "8px", borderRadius: "6px", background: "#1a2134",
		border: "1px solid " + PS_BORDER, marginBottom: "8px",
	});
	const actHead = PSEl("div", { display: "flex", alignItems: "center", gap: "8px", marginBottom: "6px" });
	actHead.appendChild(PSEl("span", { fontSize: "14px", color: PS_ACCENT, fontWeight: "700" }, "▶"));
	const actTitle = PSEl("span", { fontWeight: "700", fontSize: "13px", flex: "1" });
	actTitle.textContent = PST("actTrigger");
	actHead.appendChild(actTitle);
	const actCb = document.createElement("input");
	actCb.type = "checkbox";
	actCb.checked = !!sc.actionEnabled;
	actCb.addEventListener("change", () => { PSUpdateScript(sc.id, { actionEnabled: actCb.checked }); PSUIRenderAll(); });
	actHead.appendChild(actCb);
	actSec.appendChild(actHead);
	const actHint = PSEl("div", { fontSize: "12px", color: PS_TEXT_DIM, marginBottom: "6px" });
	actHint.textContent = PST("actHint");
	actSec.appendChild(actHint);
	const actNameInp = PSUIInput(sc.actionName, (v) => { PSUpdateScript(sc.id, { actionName: v }); PSUIRenderAll(); }, { placeholder: PST("actNamePh") });
	actSec.appendChild(PSUIEditorRow(PST("actName"), actNameInp));
	const actGroupRow = PSEl("div", { display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" });
	const actGroupLab = PSEl("label", { width: "110px", minWidth: "110px", fontSize: "13px", color: PS_TEXT_DIM });
	actGroupLab.textContent = PST("actGroup");
	actGroupRow.appendChild(actGroupLab);
	const pickBtn = PSSmallBtn("▶ " + PST("actPick"), PSUIActWinOpen);
	pickBtn.style.flex = "1";
	actGroupRow.appendChild(pickBtn);
	actGroupRow.appendChild(PSEl("span", {
		padding: "6px 10px", borderRadius: "6px", background: "#10141f",
		border: "1px solid " + PS_BORDER, fontSize: "13px", color: PS_ACCENT,
	}, PSEsc(PSActGroupLabel(sc.actionGroup || "any"))));
	actSec.appendChild(actGroupRow);
	const actTargetSel = PSUISelect(sc.actionTarget || "others", [
		'<option value="self">' + PSEsc(PST("actTargetSelf")) + "</option>",
		'<option value="others">' + PSEsc(PST("actTargetOthers")) + "</option>",
	].join(""), (v) => { PSUpdateScript(sc.id, { actionTarget: v }); PSUIRenderAll(); });
	actSec.appendChild(PSUIEditorRow(PST("actTarget"), actTargetSel));
	sec1.appendChild(actSec);

	 
	const otherSec = PSEl("div", {
		padding: "8px", borderRadius: "6px", background: "#1a2134",
		border: "1px solid " + PS_BORDER, marginBottom: "8px",
	});
	const otherHead = PSEl("div", { display: "flex", alignItems: "center", gap: "8px", marginBottom: "6px" });
	const otherTitle = PSEl("span", { fontWeight: "700", fontSize: "13px", flex: "1" });
	otherTitle.textContent = PST("otherTrigger");
	otherHead.appendChild(otherTitle);
	otherSec.appendChild(otherHead);
	const otherHint = PSEl("div", { fontSize: "12px", color: PS_TEXT_DIM, marginBottom: "6px" });
	otherHint.textContent = PST("otherHint");
	otherSec.appendChild(otherHint);
	const roomRow = PSEl("div", { display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" });
	const roomLab = PSEl("label", { width: "110px", minWidth: "110px", fontSize: "13px", color: PS_TEXT_DIM });
	roomLab.textContent = PST("roomTrigger");
	roomRow.appendChild(roomLab);
	const roomCb = document.createElement("input");
	roomCb.type = "checkbox";
	roomCb.checked = !!sc.roomTrigger;
	...
	plRow.appendChild(plLab);
	const plCb = document.createElement("input");
	plCb.type = "checkbox";
	plCb.checked = !!sc.playerTrigger;
	...
	if (sc.timeTrigger) {
		...
		tRules.forEach((r) => {
			...
			const fromInp = PSUIInput(r.from || "06:00", (v) => setTField(r.id, "from", v), { type: "text", placeholder: "06:00" });
			...
			const toInp = PSUIInput(r.to || "11:00", (v) => setTField(r.id, "to", v), { type: "text", placeholder: "11:00" });
...
function PSUINodeEditor(box, sc, node) {
	const sec2 = PSEl("div", { padding: "10px", borderRadius: "8px", background: PS_PANEL });
	sec2.appendChild(PSEl("div", { fontWeight: "700", marginBottom: "8px", color: PS_ACCENT }, PSEsc(PST("nodeEdit"))));

	 
	const typeRow = PSEl("div", { display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "10px" });
	PSTypeNames.forEach((t) => {
		const active = node.type === t;
		const b = PSEl("button", {
			padding: "8px 10px", fontSize: "13px", fontWeight: "600", fontFamily: "sans-serif",
			background: active ? PSUINodeColor(t) : "#2c3350",
			color: active ? "#10141f" : "#ffffff",
	...
	const delayInp = PSUIInput(String(node.delay), (v) => { PSUpdateNode(sc.id, node.id, { delay: v }); PSUIRenderAll(); }, { type: "number", min: "0", max: String(PS_MAX_DELAY), step: "0.1" });
	sec2.appendChild(PSUIEditorRow(PST("nodeDelay"), delayInp));
	...
	if (isLeave) {
		...
		const leaveHint = PSEl("div", {
			padding: "10px", borderRadius: "6px", background: "#2a1a1e",
			border: "1px solid #f08080", color: "#f3c3c3", fontSize: "13px", marginBottom: "8px",
		});
		leaveHint.textContent = PST("leaveHint");
		sec2.appendChild(leaveHint);
		
		const forceRow = PSEl("div", { display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" });
		const forceLab = PSEl("label", { width: "110px", minWidth: "110px", fontSize: "13px", color: PS_TEXT_DIM });
		forceLab.textContent = PST("forceLeave");
		forceRow.appendChild(forceLab);
		const forceCb = document.createElement("input");
		forceCb.type = "checkbox";
		forceCb.checked = !!node.force;
		forceCb.addEventListener("change", () => { PSUpdateNode(sc.id, node.id, { force: forceCb.checked }); PSUIRenderAll(); });
		forceRow.appendChild(forceCb);
		sec2.appendChild(forceRow);
		const forceHint = PSEl("div", { fontSize: "12px", color: PS_TEXT_DIM, marginBottom: "8px" });
		forceHint.textContent = PST("forceLeaveHint");
		sec2.appendChild(forceHint);
		PSUI.textareaEl = null;
	} else if (isOutfit) {
		
		const outfitHint = PSEl("div", {
			padding: "10px", borderRadius: "6px", background: "#1e2a1e",
			border: "1px solid #9fd8a0", color: "#cfeccf", fontSize: "13px", marginBottom: "8px",
		});
		outfitHint.textContent = PST("outfitHint");
		sec2.appendChild(outfitHint);
		const ta = document.createElement("textarea");
		ta.rows = 5;
		ta.value = node.text;
		PSStyle(ta, {
			width: "100%", boxSizing: "border-box", padding: "8px 10px", fontSize: "12px", borderRadius: "6px",
			background: "#10141f", color: PS_TEXT, border: "1px solid " + PS_BORDER,
			fontFamily: "monospace", resize: "vertical", marginBottom: "8px", wordBreak: "break-all",
		});
		const applyInput = () => {
			PSUpdateNode(sc.id, node.id, { text: ta.value });
			const card = PSUI.flowCards.find((c) => c.id === node.id);
			if (card && card.previewEl) card.previewEl.textContent = PST("outfitPreview") + "：" + PSUINodePreview(ta.value);
		};
		ta.addEventListener("input", applyInput);
		sec2.appendChild(ta);
		PSUI.textareaEl = ta;
		const outfitTargetSel = PSUISelect(node.outfitTarget === "self" ? "self" : "target", [
			'<option value="target">' + PSEsc(PST("outfitTargetTarget")) + "</option>",
			'<option value="self">' + PSEsc(PST("outfitTargetSelf")) + "</option>",
		].join(""), (v) => { PSUpdateNode(sc.id, node.id, { outfitTarget: v }); PSUIRenderAll(); });
		sec2.appendChild(PSUIEditorRow(PST("outfitTargetLabel"), outfitTargetSel));
		
		const flags = PSOutfitFlagsFromNode(node);
		const scopeRow = PSEl("div", { display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap", marginBottom: "8px" });
		const scopeLab = PSEl("label", { width: "110px", minWidth: "110px", fontSize: "13px", color: PS_TEXT_DIM });
		scopeLab.textContent = PST("outfitModeLabel");
		scopeRow.appendChild(scopeLab);
		const mkFlag = (key, labelKey, val) => {
			const box = PSEl("div", { display: "flex", alignItems: "center", gap: "4px" });
			const cb = document.createElement("input");
			cb.type = "checkbox";
			cb.checked = !!val;
			cb.addEventListener("change", () => { PSUpdateNode(sc.id, node.id, { [key]: cb.checked }); PSUIRenderAll(); });
			box.appendChild(cb);
			const lab = PSEl("span", { fontSize: "13px", color: PS_TEXT });
			lab.textContent = PST(labelKey);
			box.appendChild(lab);
			return box;
		};
		scopeRow.appendChild(mkFlag("outfitBody", "outfitBodyLabel", flags.outfitBody));
		scopeRow.appendChild(mkFlag("outfitClothes", "outfitClothesLabel", flags.outfitClothes));
		scopeRow.appendChild(mkFlag("outfitItems", "outfitItemsLabel", flags.outfitItems));
		scopeRow.appendChild(mkFlag("outfitCos", "outfitCosLabel", flags.outfitCos));
		sec2.appendChild(scopeRow);
		
		const locksRow = PSEl("div", { display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" });
		const locksLab = PSEl("label", { width: "110px", minWidth: "110px", fontSize: "13px", color: PS_TEXT_DIM });
		locksLab.textContent = PST("outfitLocksLabel");
		locksRow.appendChild(locksLab);
		const locksCb = document.createElement("input");
		locksCb.type = "checkbox";
		locksCb.checked = !!node.outfitLocks;
		locksCb.addEventListener("change", () => { PSUpdateNode(sc.id, node.id, { outfitLocks: locksCb.checked }); PSUIRenderAll(); });
		locksRow.appendChild(locksCb);
		sec2.appendChild(locksRow);
		const locksHint = PSEl("div", { fontSize: "12px", color: PS_TEXT_DIM, marginBottom: "8px" });
		locksHint.textContent = PST("outfitLocksHint");
		sec2.appendChild(locksHint);
		
		if (flags.outfitItems) {
			const modeRow = PSEl("div", { display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" });
			const modeLab = PSEl("label", { width: "110px", minWidth: "110px", fontSize: "13px", color: PS_TEXT_DIM });
			modeLab.textContent = PST("outfitItemsModeLabel");
			modeRow.appendChild(modeLab);
			const modeSel = PSUISelect(node.outfitItemsMode === "override" ? "override" : "add", [
				'<option value="add">' + PSEsc(PST("outfitItemsModeAdd")) + "</option>",
				'<option value="override">' + PSEsc(PST("outfitItemsModeOverride")) + "</option>",
			].join(""), (v) => { PSUpdateNode(sc.id, node.id, { outfitItemsMode: v }); PSUIRenderAll(); });
			modeSel.style.flex = "1";
			modeRow.appendChild(modeSel);
			sec2.appendChild(modeRow);
			...
			pickLab.textContent = PST("outfitPickLabel");
			pickRow.appendChild(pickLab);
			const pickBtn = PSSmallBtn(PST("outfitPickBtn"), PSUIOutfitWinOpen);
			pickBtn.style.flex = "1";
			pickRow.appendChild(pickBtn);
			const chipText = Array.isArray(node.outfitGroups) ? PST("outfitGroupsN", node.outfitGroups.length, 0) : PST("outfitGroupsAll");
		...
		ta.value = node.text;
		PSStyle(ta, {
			width: "100%", boxSizing: "border-box", padding: "8px 10px", fontSize: "13px", borderRadius: "6px",
			background: "#10141f", color: PS_TEXT, border: "1px solid " + PS_BORDER,
		...
		const tagRow = PSEl("div", { display: "flex", gap: "6px", alignItems: "center", flexWrap: "wrap", marginBottom: "8px" });
		...
		const tagBtn = PSSmallBtn(PST("insertNick"), () => insertToken(PS_NICK_TOKEN));
		tagBtn.title = PS_NICK_TOKEN;
	...
	const enLab = PSEl("label", { width: "110px", minWidth: "110px", fontSize: "13px", color: PS_TEXT_DIM });
	enLab.textContent = PST("nodeEnabled");
	enRow.appendChild(enLab);
	const enCb = document.createElement("input");
	enCb.type = "checkbox";
	enCb.checked = node.enabled !== false;
	enCb.addEventListener("change", () => { PSUpdateNode(sc.id, node.id, { enabled: enCb.checked }); PSUIRenderAll(); });
	enRow.appendChild(enCb);
	sec2.appendChild(enRow);
...
function PSUITimeRuleSection(box, sc, node) {
	...
	rules.forEach((r) => {
		...
		const fromInp = PSUIInput(r.from || "06:00", (v) => setField(r.id, "from", v), { type: "text", placeholder: "06:00" });
		fromInp.title = PST("timeRuleFrom");
		fromInp.style.flex = "0 1 70px";
		row1.appendChild(fromInp);
		row1.appendChild(PSEl("span", { color: PS_TEXT_DIM, fontSize: "12px" }, "~"));
		const toInp = PSUIInput(r.to || "11:00", (v) => setField(r.id, "to", v), { type: "text", placeholder: "11:00" });
		toInp.title = PST("timeRuleTo");
		toInp.style.flex = "0 1 70px";
		...
		tzInp.title = PST("timeRuleTz");
		tzInp.style.minWidth = "86px";
		tzInp.style.flex = "1 1 130px";
		row1.appendChild(tzInp);
		sec.appendChild(row1);
		
		const row2 = PSEl("div", { display: "flex", gap: "6px", alignItems: "center", marginBottom: "8px" });
		const textInp = PSUIInput(r.text || "", (v) => setField(r.id, "text", v), { placeholder: PST("timeRuleTextPh") });
		textInp.title = PST("timeRuleText");
		row2.appendChild(textInp);
...
function PSUIJudgeEditor(box, sc, node) {
	...
	const typeSel = PSUISelect(PSNormalizeJudgeType(node.judgeType), [
	...
	} else if (node.judgeType === "player") {
		const playerHint = PSEl("div", { fontSize: "12px", color: PS_TEXT_DIM, marginBottom: "8px" });
		playerHint.textContent = PST("judgePlayerHint");
		sec.appendChild(playerHint);
...
function PSUIJudgePlayerBranches(box, sc, node) {
	const sec = PSEl("div", { padding: "8px", borderRadius: "6px", background: "#141a2c", border: "1px solid " + PS_BORDER, marginBottom: "8px" });

	const branches = PSNormalizePlayerBranches(node.playerBranches);
	const commit = (next) => { PSUpdateNode(sc.id, node.id, { playerBranches: next }); PSUIRenderAll(); };
	const setIds = (bid, val) => commit(branches.map((b) => (b.id === bid ? Object.assign({}, b, { ids: PSNormalizePlayerIds(String(val || "").split(/[、,，\s]+/).map((x) => Number(x))) }) : b)));
	const del = (bid) => commit(branches.filter((b) => b.id !== bid));

	branches.forEach((b, i) => {
		const row = PSEl("div", { display: "flex", gap: "6px", alignItems: "center", marginBottom: "6px" });
		row.appendChild(PSEl("span", { width: "46px", minWidth: "46px", fontSize: "12px", color: "#f0b3ff" }, PSEsc(PST("judgePlayerBranchPh", i + 1))));
		const idsInp = PSUIInput((b.ids || []).join("、"), (v) => setIds(b.id, v), { type: "text", placeholder: PST("judgePlayerIdsPh") });
		idsInp.style.flex = "1 1 110px";
		idsInp.style.minWidth = "80px";
		row.appendChild(idsInp);
		const t = b.nodeId ? PSUINodeTargetLabel(sc, b.nodeId) : null;
		if (t) {
			...
			info.title = t;
			row.appendChild(info);
			const disc = PSSmallBtn(PST("connectDisconnect"), () => { PSNodeDisconnect(sc.id, node.id, "player:" + b.id); PSUIRenderAll(); });
			row.appendChild(disc);
		} else {
			const btn = PSSmallBtn(PST("connectBtn"), () => PSUIConnectWinOpen(sc.id, node.id, "player:" + b.id));
			row.appendChild(btn);
		}
		const delBtn = PSSmallBtn("✕", () => del(b.id), { bg: "#5a2c3a" });
		row.appendChild(delBtn);
	...
	const elseRow = PSEl("div", { display: "flex", gap: "6px", alignItems: "center", marginBottom: "6px" });
	elseRow.appendChild(PSEl("span", { width: "46px", minWidth: "46px", fontSize: "12px", color: "#f0b3ff" }, PSEsc(PST("judgePlayerElse"))));
	const t = node.elseId ? PSUINodeTargetLabel(sc, node.elseId) : null;
	if (t) {
		const info = PSEl("span", { flex: "1", minWidth: "0", fontSize: "12px", color: PS_ACCENT, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }, PSEsc(t));
		info.title = t;
		elseRow.appendChild(info);
		const disc = PSSmallBtn(PST("connectDisconnect"), () => { PSNodeDisconnect(sc.id, node.id, "else"); PSUIRenderAll(); });
		elseRow.appendChild(disc);
	} else {
		const btn = PSSmallBtn(PST("connectBtn"), () => PSUIConnectWinOpen(sc.id, node.id, "else"));
		elseRow.appendChild(btn);
		elseRow.appendChild(PSEl("span", { fontSize: "12px", color: PS_TEXT_DIM }, PSEsc(PST("connectHint"))));
	}
	sec.appendChild(elseRow);

	box.appendChild(sec);
}

 
function PSUIJudgeRelationBranches(box, sc, node) {
	const sec = PSEl("div", { padding: "8px", borderRadius: "6px", background: "#141a2c", border: "1px solid " + PS_BORDER, marginBottom: "8px" });

	const branches = PSNormalizeRelationBranches(node.relationBranches);
	const commit = (next) => { PSUpdateNode(sc.id, node.id, { relationBranches: next }); PSUIRenderAll(); };
	const setRel = (bid, rel) => commit(branches.map((b) => (b.id === bid ? Object.assign({}, b, { rel }) : b)));
	const del = (bid) => commit(branches.filter((b) => b.id !== bid));

	branches.forEach((b) => {
		const row = PSEl("div", { display: "flex", gap: "6px", alignItems: "center", marginBottom: "6px" });
		const relSel = PSUISelect(b.rel, PS_RELATION_ORDER.map((r) => '<option value="' + r + '">' + PSEsc(PSRelationLabel(r)) + "</option>").join(""), (v) => setRel(b.id, v));
		relSel.style.flex = "1 1 110px";
		relSel.style.minWidth = "84px";
		...
		if (t) {
			...
			info.title = t;
			row.appendChild(info);
			const disc = PSSmallBtn(PST("connectDisconnect"), () => { PSNodeDisconnect(sc.id, node.id, "relation:" + b.id); PSUIRenderAll(); });
			row.appendChild(disc);
		} else {
			const btn = PSSmallBtn(PST("connectBtn"), () => PSUIConnectWinOpen(sc.id, node.id, "relation:" + b.id));
			row.appendChild(btn);
		}
		const delBtn = PSSmallBtn("✕", () => del(b.id), { bg: "#5a2c3a" });
		row.appendChild(delBtn);
...
function PSUIJudgeCountSection(box, sc, node) {
	...
	const modeSel = PSUISelect(node.countMode === "interact" ? "interact" : (node.countMode === "orgasm" ? "orgasm" : (node.countMode === "node" ? "node" : "script")), [
		...
		'<option value="node">' + PSEsc(PST("judgeCountNode")) + "</option>",
	...
	if (node.countMode === "interact") {
		...
		sec.appendChild(hint);
	} else if (node.countMode === "node") {
		const hint = PSEl("div", { fontSize: "12px", color: PS_TEXT_DIM, marginBottom: "8px", lineHeight: "1.5" });
		hint.textContent = PST("judgeCountNodeHint");
		sec.appendChild(hint);
	} else {
		const pickRow = PSEl("div", { display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" });
		const pickLab = PSEl("label", { width: "110px", minWidth: "110px", fontSize: "13px", color: PS_TEXT_DIM });
...
function PSUIRenderStatus() {
	...
	if (PSActive) {
		const A = PSActive;
		const remain = Math.max(0, A.nextAt - PSNow());
		html = PST("statusRunning", PSEsc(A.name), A.idx + 1, A.nodes.length, (remain / 1000).toFixed(1));
	} else if (PSQueue.length) {
		html = PST("statusQueued", PSQueue.length);
	} else {
		html = PST("statusIdle");
	}
	PSUI.statusEl.innerHTML = html;
}

 

function PSUIImportBuild(root) {
	const overlay = PSEl("div", {
		position: "absolute", left: "0", top: "0", right: "0", bottom: "0",
		background: "rgba(10,12,20,.92)", zIndex: "5", display: "none",
		padding: "24px", overflowY: "auto",
	});
	const panel = PSEl("div", {
		maxWidth: "560px", margin: "0 auto", padding: "16px", borderRadius: "10px",
		background: PS_PANEL, border: "1px solid #ffffff",
	});
	panel.appendChild(PSEl("div", { fontWeight: "700", fontSize: "16px", marginBottom: "8px", color: PS_ACCENT }, PSEsc(PST("importTitle"))));
	panel.appendChild(PSEl("div", { color: PS_TEXT_DIM, fontSize: "13px", marginBottom: "10px" }, PSEsc(PST("importHint"))));
	const ta = document.createElement("textarea");
	ta.rows = 8;
	PSStyle(ta, {
		width: "100%", boxSizing: "border-box", padding: "10px", fontSize: "12px", borderRadius: "6px",
		background: "#10141f", color: PS_TEXT, border: "1px solid " + PS_BORDER,
		fontFamily: "monospace", resize: "vertical", marginBottom: "10px",
	});
	panel.appendChild(ta);
	const fileInp = document.createElement("input");
	fileInp.type = "file";
	fileInp.accept = ".txt,.json";
	...
	fileInp.addEventListener("change", () => {
		...
		try {
			if (typeof FileReader === "function") {
				...
				fr.onload = () => { ta.value = String(fr.result || ""); PSUIImportApply(ta.value); };
				fr.readAsText(f);
			} else {
				PSUI.importFileEl = f;
			}
		} catch (e) { PSErr("读取文件失败", e); }
	});
	panel.appendChild(fileInp);
	const btnRow = PSEl("div", { display: "flex", gap: "8px" });
	const goBtn = PSBigBtn(PST("importFromText"), () => PSUIImportApply(ta.value), { bg: "#2c7a70" });
	const closeBtn = PSBigBtn(PST("importClose"), PSUIImportToggle);
	btnRow.appendChild(goBtn);
	btnRow.appendChild(closeBtn);
	panel.appendChild(btnRow);
	overlay.appendChild(panel);
	root.appendChild(overlay);
	PSUI.importEl = overlay;
	PSUI.importTextEl = ta;
	PSUI.importFileEl = fileInp;
}

function PSUIImportApply(text) {
	if (!String(text || "").trim()) { PSToast(PST("toastImportBad")); return; }
	const r = PSImportJSON(text);
	if (!r) { PSToast(PST("toastImportBad")); return; }
	PSToast(PST("toastImported", r.added, r.replaced));
	PSUIImportToggle();
	PSUIRenderAll();
}

function PSUIImportToggle() {
	if (!PSUI.importEl) return;
	const show = PSUI.importEl.style.display !== "flex";
	PSUI.importEl.style.display = show ? "flex" : "none";
}
...
function PSUIConnectWinRender() {
	...
	if (port === "yes") portLabel = PST("connectYes");
		...
		const idx = node && Array.isArray(node.playerBranches) ? node.playerBranches.findIndex((b) => b.id === bid) : -1;
		portLabel = PST("judgePlayerBranchPh", idx >= 0 ? idx + 1 : 1);
	}
	else if (typeof port === "string" && port.indexOf("relation:") === 0) {
		const bid = port.slice("relation:".length);
		const node = sc.nodes.find((n) => n.id === PSUI.connectNodeId);
...
function PSUIDownloadText(filename, text) {
	try {
		...
		return true;
	} catch (e) { PSErr("导出文件失败", e); return false; }
}

function PSUIPickTextFile(cb) {
	try {
		...
		inp.addEventListener("change", () => {
			...
			r.onload = () => { try { cb(String(r.result || ""), f.name); } catch (e) { PSErr("读取文件失败", e); } inp.remove(); };
			r.onerror = () => { PSErr("读取文件失败"); inp.remove(); };
			r.readAsText(f, "utf-8");
		});
		inp.click();
	} catch (e) { PSErr("选择文件失败", e); }
}

function PSCloudDetailEntry(key) {
	return PSCloudDetailEntries().find((x) => x.key === key) || null;
}
...
function PSCloudDetailRestore(e, value) {
	if (!e || typeof e.top !== "string") return false;
	try {
		if (!PSStore.lastLoginData || typeof PSStore.lastLoginData !== "object") PSStore.lastLoginData = {};
		const top = e.top, sub = e.sub;
		if (sub) {
			if (!PSStore.lastLoginData[top] || typeof PSStore.lastLoginData[top] !== "object") PSStore.lastLoginData[top] = {};
			PSStore.lastLoginData[top][sub] = value;
		} else {
			PSStore.lastLoginData[top] = value;
		}
		if (typeof Player !== "undefined" && Player) {
			if (sub) {
		...
		PSToast(PST("cloudImportDone"));
		return true;
	} catch (e3) { PSErr("恢复失败", e3); return false; }
}

function PSCloudDetailImport(key) {
	const e = PSCloudDetailEntry(key);
	if (!e) return;
...
function PSUICloudDetailWinRender() {
	...
	if (entries.length) {
		for (const e of entries) {
			const r = PSEl("div", { display: "flex", gap: "8px", alignItems: "center", padding: "7px 0", borderBottom: "1px solid " + PS_BORDER, fontSize: "13px" });
			const l = PSEl("div", { width: "180px", minWidth: "180px", color: PS_TEXT_DIM, wordBreak: "break-all" });
			l.textContent = e.label;
			r.appendChild(l);
			const v = PSEl("div", { flex: "1", textAlign: "right", color: PS_TEXT_DIM, wordBreak: "break-all" });
			v.textContent = kb(e.bytes) + "（" + (isFinite(Number(e.bytes)) ? e.bytes + " 字节" : "—") + "）";
			r.appendChild(v);
			const expBtn = PSSmallBtn(PST("cloudExportBtn"), () => PSCloudDetailExport(e.key));
			const impBtn = PSSmallBtn(PST("cloudImportBtn"), () => PSCloudDetailImport(e.key));
			r.appendChild(expBtn);
			r.appendChild(impBtn);
			box.appendChild(r);
		}
	} else {
...
function PSUIRenderAll() {
	...
	if (PSUI.btnNew) PSUI.btnNew.textContent = PST("newScript");
	if (PSUI.btnTest) PSUI.btnTest.textContent = PST("testRun");
	if (PSUI.btnStop) PSUI.btnStop.textContent = PST("stop");
	if (PSUI.btnExport) PSUI.btnExport.textContent = PST("exportBtn");
	if (PSUI.btnImport) PSUI.btnImport.textContent = PST("importBtn");
	if (PSUI.btnCloud) PSUI.btnCloud.textContent = PST("cloudCheckBtn");
	if (PSUI.cloudCapInp && PSStore.state) PSUI.cloudCapInp.value = String(PSStore.cloudCapKB());
	if (PSUI.btnClear && !PSUI.armed["clearall"]) PSUI.btnClear.textContent = PST("clearAll");
	if (PSUI.focusRequest && PSUI.textareaEl) {
		PSUI.focusRequest = false;
		try { PSUI.textareaEl.focus(); } catch (e) {   }
	}
}

function PSUILangToggle() {
	PSUI.lang = PSUI.lang === "zh" ? "en" : "zh";
	PSStorage.set("PlayScriptLang", PSUI.lang);
	PSUIRenderAll();
}

 

function PlayScriptOpen() {
	if (PSUI.open) return;
	...
	if (!el) return;
	PSUIDotBuild();
	PSUI.win.minimized = false;
	PSUIWinClamp();
	PSStyle(el, { left: PSUI.win.x + "px", top: PSUI.win.y + "px", width: PSUI.win.w + "px", height: PSUI.win.h + "px" });
	el.style.display = "block";
	PSUI.bodyEl.style.display = "flex";
	PSUI.toolbarEl.style.display = "flex";
	PSUI.statusEl.style.display = "block";
	PSUI.cornerEl.style.display = "block";
	PSUIRenderAll();
	if (!PSUI.tickId) {
		PSUI.tickId = setInterval(() => {
			if (PSActive) PSUIRenderStatus();
		}, 250);
	}
}

function PlayScriptClose() {
	if (!PSUI.open) return;
	PSUI.open = false;
	if (PSUI.tickId) { clearInterval(PSUI.tickId); PSUI.tickId = null; }
	if (PSUI.el) PSUI.el.style.display = "none";
}

function PlayScriptToggle() {
	if (PSUI.open) PlayScriptClose();
	else PlayScriptOpen();
}

 

function PSUINewScript() {
	if (!PSStore.state) return;
	const s = PSAddScript("", "");
	if (s) {
		PSUI.selScriptId = s.id;
		PSUI.selNodeId = null;
		PSUIRenderAll();
	}
}

function PSUITestRun() {
	const sc = PSFindScript(PSUI.selScriptId);
	if (!sc) { PSToast(PST("toastNoSelScript")); return; }
	if (!sc.keyword) PSToast(PST("toastKwEmpty"));
	const r = PSFire(sc.id, { manual: true });
	if (r && r.ok) PSUIRenderStatus();
}

 

function PSExposeAPI() {
	const g = (typeof globalThis !== "undefined") ? globalThis
		: (typeof window !== "undefined" ? window : null);
	if (!g) return;
	const api = {
		Open: PlayScriptOpen,
		Close: PlayScriptClose,
		Toggle: PlayScriptToggle,
		State: () => ({
			open: PSUI.open,
			running: !!PSActive,
			active: PSActive ? {
				scriptId: PSActive.scriptId, name: PSActive.name,
				idx: PSActive.idx, total: PSActive.nodes.length, nextAt: PSActive.nextAt,
			} : null,
			queue: PSQueue.slice(),
			scripts: (PSStore.state ? PSStore.state.scripts.length : 0),
		}),
		AddScript: (name, keyword) => PSAddScript(name, keyword),
		DeleteScript: (id) => PSDeleteScript(id),
		DuplicateScript: (id) => PSDuplicateScript(id),
		RenameScript: (id, name) => PSUpdateScript(id, { name }),
		UpdateScript: (id, patch) => PSUpdateScript(id, patch),
		AddNode: (scriptId, node, atIdx) => PSAddNode(scriptId, node, atIdx),
		...
		IsRunning: () => !!PSActive,
		Version: () => PS_VERSION,
		LastOutfitBlocked: () => PSLastOutfitBlocked.slice(),
		LastOutfitCleared: () => PSLastOutfitCleared.slice(),
		...
		DebugOutfit: (code) => {
			...
			const family = (typeof Player !== "undefined" && Player && typeof Player.AssetFamily === "string" && Player.AssetFamily) ? Player.AssetFamily : "Female3DCG";
			const flags = PSOutfitFlagsFromNode({});
			const items = bundle.map((b) => {
				const cls = PSOutfitClassOf(b, family);
				return {
					Group: b.Group, Name: b.Name, cls,
					checkedByDefault: PSOutfitClassAllowed(cls, flags),
					assetFound: (typeof AssetGet === "function") ? !!(AssetGet(family, b.Group, b.Name)) : null,
				};
			});
	...
	g.PlayScript = api;
	g.PlayScriptOpen = PlayScriptOpen;
	g.PlayScriptClose = PlayScriptClose;
	g.PlayScriptToggle = PlayScriptToggle;
}
...
if (typeof module !== "undefined" && module.exports) {
	module.exports = {
		...
		PSNormalizeCountBranches, PSJudgeAddCountBranch, PSCounterResetIfDue, PSCounterValue, PSCounterReset, PSCounterRecordScript, PSCounterRecordInteract, PSOrgasmCountNow, PSCounterRecordOrgasm, PSCounterRecordNode,
		...
		PSDebugOutfit: (code) => {
			...
			const family = (typeof Player !== "undefined" && Player && typeof Player.AssetFamily === "string" && Player.AssetFamily) ? Player.AssetFamily : "Female3DCG";
			return {
				version: PS_VERSION, family,
				items: bundle.map((b) => {
					const cls = PSOutfitClassOf(b, family);
					return { Group: b.Group, Name: b.Name, cls, assetFound: (typeof AssetGet === "function") ? !!(AssetGet(family, b.Group, b.Name)) : null };
				}),
			};
		},
		PSActive: () => PSActive, PSQueueOf: () => PSQueue.slice(), PSLastFireOf: () => PSLastFire,
	};
}
