...
function PSDeleteNode(scriptId, nodeId) {
	...
	sc.nodes = sc.nodes.filter((n) => n.id !== nodeId);
	if (sc.nodes.length === before) return false;
	if (PSUI.selNodeId === nodeId) PSUI.selNodeId = null;
	PSStore.requestSave();
	return true;
}
...
function PSTriggerTargetOf(text, sc, match) {
	...
	const push = (arr) => {
		...
		for (const C of arr) {
			if (C && typeof C.MemberNumber === "number" && typeof C.Name === "string" && C.Name) members.push(C);
		}
	};
	...
	const matchOne = (candidate) => {
		...
		for (const C of members) {
			if (eq(C.Name, candidate)) return { num: C.MemberNumber, name: C.Name };
		}
		if (/^\d+$/.test(candidate)) {
			const num = Number(candidate);
			for (const C of members) {
				if (typeof C.MemberNumber === "number" && C.MemberNumber === num) return { num: C.MemberNumber, name: C.Name };
			}
		}
		return null;
	};
...
function PSUIWinSave() {
	if (!PSStore.state) return;
	PSStore.state.ui.win = { x: PSUI.win.x, y: PSUI.win.y, w: PSUI.win.w, h: PSUI.win.h, minimized: PSUI.win.minimized };
	PSStore.requestSave();
}
